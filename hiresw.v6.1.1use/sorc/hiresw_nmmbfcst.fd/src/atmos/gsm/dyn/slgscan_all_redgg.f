...
!
      ...
      use gfs_dyn_resol_def    , only : latg,levs,lonf,ntrac
      use namelist_dynamics_def, only : redgg_a, phigs
...
!Moor use layout_grid_tracers , only : yhalo
      use  gfs_dyn_layout1    , only : ipt_lats_node_a,lats_dim_a
...
!
     ...
     &                                   plev,plon,plond,pi
      ...
      use namelist_dynamics_def , only : redgg_a
!
      implicit none
...
!     Code adapted from subroutine dep3dg_h
     ...
     &                                  plev,plon,plond,pi
      use slgshr               , only : dlam,rdlam
      use namelist_dynamics_def, only : redgg_a,iter_one_no_interp
!
      implicit none
