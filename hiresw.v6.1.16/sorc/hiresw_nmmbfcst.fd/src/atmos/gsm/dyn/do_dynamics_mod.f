...
!$omp parallel do private(lan)
!$omp+private(lat,lon_dim,lons_lat,jlonf,i,k,ilan,rcs_loc)
...
!       anl_gr_a_2(1,1,lan) = -999
!     write(0,*)' in do_dynamics_gridn2anl_slg lan=',lan
!    &,' jlonf=',jlonf,' lons_lat=',lons_lat,' lota=',
!    &lota,' kau=',kau,' kav=',kav,' kat=',kat,' kar=',kar
!    &,' anlsize=',size(anl_gr_a_2,1),size(anl_gr_a_2,2),
!    &size(anl_gr_a_2,3),' sizeg=',size(grid_gr,1),
!    &size(grid_gr,2),' anl=',anl_gr_a_2(1,1,lan)

        do k=1,levs
!      write(0,*)' in do_dynamics_gridn2anl_slg k=',k
!    &,' g_u=',g_u,' g_v=',g_v,' g_t=',g_t

          do i=1,lons_lat
            ilan = i+jlonf

!     write(0,*)' i=',i,' ilan=',ilan,' k=',k,' grid_gru=',
!    &grid_gr(ilan,g_u  +k-1)

            anl_gr_a_2(i,kau-1+k,lan) = grid_gr(ilan,g_u+k-1) * rcs_loc
            anl_gr_a_2(i,kav-1+k,lan) = grid_gr(ilan,g_v+k-1) * rcs_loc

!     if (i == 1)
!    &write(0,*)' i=',i,' ilan=',ilan,' k=',k,' grid_grv=',
!    &grid_gr(ilan,g_v  +k-1),' grid_gru=',grid_gr(ilan,g_u+k-1),
!    &' rcs_loc=',rcs_loc,anl_gr_a_2(i,kau-1+k,lan),
!    &anl_gr_a_2(i,kav-1+k,lan),grid_gr(ilan,g_t+k-1)


!     write(0,*)' i=',i,' ilan=',ilan,' k=',k,' grid_grt=',
!    &grid_gr(ilan,g_t  +k-1)
...
!
        if( .not. ndslfv) then
          do k=1,levh
            do i=1,lons_lat
              ilan=i + jlonf
              anl_gr_a_2(i,kar-1+k,lan) = grid_gr(ilan,g_rt +k-1)
!     if (k == levh) then
!     write(150+me,*)' anl_gr_a_2=',anl_gr_a_2(i,kar-1+k,lan)
!    &,' grid_gr=',grid_gr(ilan,g_rt +k-1),' i=',i,' lan=',lan
!    &,' ilan=',ilan
!     endif
            enddo
          enddo
...
!     write(0,*)' Uanl=',anl_gr_a_2(1,kau:kau+levs-1,1)
!     write(0,*)' vanl=',anl_gr_a_2(1,kav:kav+levs-1,1)
!     write(0,*)' tanl=',anl_gr_a_2(1,kat:kat+levs-1,1)
!     write(0,*)' panl=',anl_gr_a_2(1,kap,1)
...
!--------------------------------------------
     ...
     &      'at kst=',kst
      ...
      real(kind=kind_evod) trie_ls(len_trie_ls,2,lotls)
      real(kind=kind_evod) trio_ls(len_trio_ls,2,lotls)
      real(kind=kind_evod), parameter :: pi=acos(-1.0)
      integer kst,nst
...
!     write(0,*)' in do_dynamics_syn2gridn_slg syn = ',
!    &(syn_gr_a_2(1,kst -1+k,1), k=1,10),' kst=',kst
...
!     write(150+me,*)' in dynamics lan=',lan,' lat=',lat,' lons_lat=',
!    &lons_lat,' ipt_lats_node_a=',ipt_lats_node_a,' me=',me
!    &,' lats_node_a=',lats_node_a,' lats_dim_a=',lats_dim_a
!
        do i=1,lons_lat
            ilan = i + jlonf
            grid_gr(ilan,G_zq) = syn_gr_a_2(i,ksq,lan)
!           grid_gr(ilan,G_zq) =  exp(syn_gr_a_2(i,ksq,lan))
...
!           grid_gr(ilan,G_dpn+k-1) = syn_gr_a_2(i,ksdp-1+k,lan)
          ...
          enddo
        enddo
!     write(121,*)' grid_gr=',grid_gr(jlonf+lons_lat,g_dpn:g_dpn+2)
!     write(0,*)' grid_gr=',grid_gr(1,g_dpn:g_dpn+levs-1)
!
        do k=1,levh
          do i=1,lons_lat
            ilan = i + jlonf
...
!
      enddo
!     write(150+me,*)' grid_gr maxsyn=',
!    &maxval(syn_gr_a_2(:,ksr+levh-1,:))
!    &, ' minsyn=',minval(syn_gr_a_2(:,ksr+levh-1,:))
!    &,' lan=',lan,' ksr+levh-1=',ksr+levh-1
!     write(150+me,*)' in do_dynamics_syn2gridn_slg gridgr=',
!    &grid_gr(1,g_rt+levh-1),syn_gr_a_2(1,ksr+levh-1,1),' me=',me
...
!     write(0,*)' lats_node_a=',lats_node_a,'lats_dim_ext= '
!    &,lats_dim_ext
      do lan=1,lats_node_a
!       write(0,*)' lan= ',lan,' ipt_lats_node_a=',ipt_lats_node_a
        lat      = global_lats_a(ipt_lats_node_a-1+lan)
        lon_dim  = lon_dims_a(lan)
        lons_lat = lonsperlat(lat)
        jlonf    = (lan-1)*lonf
...
!     write(0,*)' in do_dynamics_gridomega lan=',lan,' lat=',lat
!    &,' lon_dim=',lon_dim,' ngptc=',ngptc
...
!     write(0,*)' lon=',lon,' njeff=',njeff,' ngptc=',ngptc,' lan=',lan
          do k=1,levs
            do i=1,njeff
              ii = lon - 1 + i
...
!     write(0,*)' RETURNING FROM  do_dynamics_gridomega'
      return
...
!     write(0,*)' in do_dynamics_gridomega lan=',lan,' lat=',lat
!    &,' calling omega_hyb', ' gq=',gq(1),' ugr=',ugr(1,1:5),
!    &' vgr=',vgr(1,1:5),' gphi=',gphi(1),' glam=',glam(1)
!    &,' gd=',gd(1,1:5)
...
!     write(0,*)' RETURNING FROM  do_dynamics_gridomega_slg'
      return
...
!$omp+private(lat,lons_lat,jlonf,i,k,ilan)
      ...
      do lan=1,lats_node_a
        ...
        do k=1,levh
          ...
          enddo
        enddo
!     write(150+me,*)' In do_dynamics_gridp2n gridimx=',
!    &maxval(grid_gr(:,g_rt+levh-1)),'gridmin=',
!    &minval(grid_gr(:,g_rt+levh-1)),' g_rt_levh-1=',g_rt+levh-1
        do i=1,lons_lat
          ilan = i + jlonf
...
!     write(0,*)' G_u=',G_u,' G_v=',G_v,' G_t=',G_t,' G_dpn=',G_dpn,
!    &' G_rt=',G_rt,' G_zq=',G_zq
!     write(0,*)' Ugrd=',grid_gr(1,g_u:g_u+levs-1)
!     write(0,*)' vgrd=',grid_gr(1,g_v:g_v+levs-1)
!     write(0,*)' tgrd=',grid_gr(1,g_t:g_t+levs-1)
!     write(0,*)' pgrd=',grid_gr(1,g_zq)
...
!      write(0,*)' in do_dynamics_gridpdp,gq=',gq(1),' lan=',lan
!      write(0,*)' in do_dynamics_gridpdp,gtv=',gtv(1,:),' lan=',lan
          if( gen_coord_hybrid ) then 
            call gch2press(njeff,ngptc,gq, gtv, prsl, dprs)
          else if( hybrid ) then
            call hyb2press(njeff,ngptc,gq, prsl, dprs)
...
!      write(0,*)' in do_dynamics_gridpdp-prsl=',prsl(1,:),' lan=',lan
          do k=1,levs
            do i=1,njeff
