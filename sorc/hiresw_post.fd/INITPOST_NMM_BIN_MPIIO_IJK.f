...
! start calling mpi io
      ...
      if (ierr /= 0) then
       write(0,*) 'ierr from mpi_file_open: ', ierr
       write(0,*) 'Error opening file ', trim(filename), ' with mpi io'
...
!!!!!!!!!!!!!!!!!
      ...
      if (iret /= 0) then
        ...
        this_length=im*(jend_2u-jsta_2l+1)
        call mpi_file_read_at(iunit,this_offset &
       ,max10u,this_length,mpi_real4 &
       , mpi_status_ignore, ierr)
        if (ierr /= 0) then
          print*,"Error reading ", VarName,"Assigned missing values"
          max10u=SPVAL
        else
           print*, 'read MAX10U properly ', &
                   minval(max10U),maxval(max10u)
        end if
...
!!!!!!!!!!!!!!!!!
      ...
      if (iret /= 0) then
        ...
        call mpi_file_read_at(iunit,this_offset &
      , max10v,this_length,mpi_real4 &
      , mpi_status_ignore, ierr)
        if (ierr /= 0) then
          print*,"Error reading ", VarName,"Assigned missing values"
          max10v=SPVAL
        else
           print*, 'read MAX10V properly ', &
                   minval(max10v),maxval(max10v)
        end if
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!
      ...
      if (iret /= 0) then
        ...
        this_length=im*(jend_2u-jsta_2l+1)
        call mpi_file_read_at(iunit,this_offset &
       ,w_up_max,this_length,mpi_real4 &
       , mpi_status_ignore, ierr)
        if (ierr /= 0) then
          print*,"Error reading ", VarName,"Assigned missing values"
          w_up_max=SPVAL
        else
           print*, 'read MAXUPDR properly ', &
     &              minval(w_up_max),maxval(w_up_max)
        end if

        do j=jsta_2l, jend_2u
        do I=1,im
        if (mod(I,60) .eq. 0 .and. mod(J,60) .eq. 0) then
        write(0,*) 'I,J, MAXUPDR: ', I,J, w_up_max(I,J)
        endif
        enddo
        enddo
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!
      ...
      if (iret /= 0) then
        ...
        this_length=im*(jend_2u-jsta_2l+1)
        call mpi_file_read_at(iunit,this_offset &
       ,w_dn_max,this_length,mpi_real4 &
       , mpi_status_ignore, ierr)
        if (ierr /= 0) then
          print*,"Error reading ", VarName,"Assigned missing values"
          w_dn_max=SPVAL
        else
           print*, 'read MAXDNDR properly ', &
                   minval(w_dn_max),maxval(w_dn_max)
        end if

        do j=jsta_2l, jend_2u
        do I=1,im
        if (mod(I,60) .eq. 0 .and. mod(J,60) .eq. 0) then
        write(0,*) 'I,J, MAXDNDR: ', I,J, w_dn_max(I,J)
        endif
        enddo
        enddo
...
!!!!!!!!!!!!!!!!!
      ...
      if (iret /= 0) then
        ...
        this_length=im*(jend_2u-jsta_2l+1)
        call mpi_file_read_at(iunit,this_offset &
       ,up_heli_max,this_length,mpi_real4 &
       , mpi_status_ignore, ierr)
        if (ierr /= 0) then
          print*,"Error reading ", VarName,"Assigned missing values"
          up_heli_max=SPVAL
        else
           print*, 'read MAXHLCY properly ', &
                   minval(up_heli_max),maxval(up_heli_max)
        end if
...
!!!!!!!!!!!!!!!!!
      ...
      if (iret /= 0) then
        ...
        this_length=im*(jend_2u-jsta_2l+1)
        call mpi_file_read_at(iunit,this_offset &
       ,refd_max,this_length,mpi_real4 &
       , mpi_status_ignore, ierr)
        if (ierr /= 0) then
          print*,"Error reading ", VarName,"Assigned missing values"
          refd_max=SPVAL
        else
           print*, 'read MAXDBZ properly ', &
                   minval(refd_max),maxval(refd_max)
        end if
