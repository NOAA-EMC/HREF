...
C
     ...
     &,T(IM,jsta_2l:jend_2u,LM),Q(IM,jsta_2l:jend_2u,LM)
      DIMENSION PDSL(IM,JM),HTM(IM,jsta_2l:jend_2u,LM),KARR(IM,JM)
                             D I M E N S I O N
...
C-----------------------------------------------------------------------
      ...
      IF(KARR(I,J).GT.0)THEN
        ...
        TBTK  =T(I,J,L)
        QBTK  =Q(I,J,L)
        PRESK=PDTOP*SGML1(L)+PDSL(I,J)*SGML2(L)+PT
...
C**
      IF(KNUML.GT.0)THEN
        CALL TTBLEX(TWET(1,jsta_2l,L),TTBL,ITB,JTB,KLRES,PDSL,SGML1(L)
...
C**
      IF(KNUMH.GT.0)THEN
       CALL TTBLEX(TWET(1,jsta_2l,L),TTBLQ,ITBQ,JTBQ,KHRES,PDSL,SGML1(L)
     1,SGML2(L),PDTOP,HTM(1,jsta_2l,L),PT,PLQ,QQ,PP,RDPQ,THE0Q,STHEQ
