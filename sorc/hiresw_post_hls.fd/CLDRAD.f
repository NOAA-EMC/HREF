...
C
              ...
              ELSE IF (CBOT.EQ.LM) THEN
                 CLDBP(I,J) = SGML1(INT(CBOT))*PDTOP
     +		 +SGML2(INT(CBOT))*PDSL(I,J)+PT1
                 CLDBZ(I,J) = ZINT(I,J,LM) 
              ELSE
                 LEV = CBOT
                 CLDBP(I,J) = SGML2(INT(LEV))*PDSL(I,J)+PT1
     +           	      +SGML1(INT(LEV))*PDTOP	 
...
C
            ...
            ELSE IF (CTOP.EQ.LM) THEN
              CLDTP(I,J) = SGML2(INT(LM))*PDSL(I,J)+PT1
     +	                  +SGML1(INT(LM))*PDTOP 
              CLDTZ(I,J) = ZINT(I,J,LM)
              CLDTT(I,J) = T(I,J,LM)
            ELSE
              LEV = CTOP
              CLDTP(I,J) = SGML2(INT(LEV))*PDSL(I,J)+PT1
     +                    +SGML1(INT(LEV))*PDTOP	      
