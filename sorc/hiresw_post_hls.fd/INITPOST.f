...
cwas     READ(LUNHB)((HTM(I,J,L),I=1,IM),J=1,JM)
         ...
         CALL EXTRACT_P(DUMMY,HTM(1,jsta_2l,L))
      END DO
      DO L=1,LM
cwas     READ(LUNHB)((VTM(I,J,L),I=1,IM),J=1,JM)
         READ(LUNHB)((DUMMY(I,J),I=1,IM),J=1,JM)
         CALL EXTRACT_P(DUMMY,VTM(1,jsta_2l,L))
      END DO
c      READ(LUNHB) DY,CPGFV,EN,ENT,R,PT,TDDAMP,F4D,F4Q,
c     X     EF4T,PDTOP,DETAD,RDETA,AETAD,F4Q2,ETAD,DFL
c     X     ,dsg1,sgml1,sg1
c     X     ,dsg2,sgml2,sg2,EM,EMT
      READ(LUNHB) DY,CPGFV,EN,ENT,R,PT,TDDAMP,F4D,F4Q,
     X     EF4T,PDTOP,DETAD,AETAD,F4Q2,ETAD,DFL
     X     ,dsg1,sgml1,sg1
     X     ,dsg2,sgml2,sg2,EM,EMT
      PDTOP2=PDTOP
cwas  READ(LUNHB) DX
      READ(LUNHB) DUMMY
...
cwas  READ(LUNHB) DDMPV
     ...
     X     RDTHE,DETA2,AETA2,DFRLG
     X     ,dsg12,sgml12,dsg22,sgml22,QS0,SQS,STHE,THE0
cwas  READ(LUNHB) MXSNAL 
      READ(LUNHB) DUMMY
      CALL EXTRACT(DUMMY,MXSNAL)
...
cwas  READ(LUNHB) HDACV
     ...
     X     ETA1,dsg1,sgml1,sg1,dsg2,sgml2,sg2
...
cwas    READ(LRSTRT) ((OMGA(I,J,L),I=1,IM),J=1,JM)
        ...
        CALL EXTRACT_P(DUMMY,OMGA(1,jsta_2l,L))
      END DO
C***
      IF(ME.EQ.0)THEN
...
CWAS  READ(LRSTRT) PD,RES,FIS
      READ(LRSTRT) DUMMY
      CALL EXTRACT_P(DUMMY,PD)
      BACKSPACE LRSTRT
      READ(LRSTRT) DUMMY,DUMMY
      CALL EXTRACT_P(DUMMY,RES)
      BACKSPACE LRSTRT
      READ(LRSTRT) DUMMY,DUMMY,DUMMY
      CALL EXTRACT_P(DUMMY,FIS)
C
CWAS      READ(LRSTRT) PDB,TB,QB,UB,VB
      READ(LRSTRT) PDB,TB,QB,UB,VB,Q2B,CWMB
C***
      DO L = 1,LM
cwas    READ(LRSTRT) ((T(I,J,L),I=1,IM),J=1,JM)
        READ(LRSTRT) ((DUMMY(I,J),I=1,IM),J=1,JM)
        CALL EXTRACT_P(DUMMY,T(1,jsta_2l,L))
      END DO
      DO L = 1,LM	
cwas    READ(LRSTRT) ((Q(I,J,L),I=1,IM),J=1,JM)
        READ(LRSTRT) ((DUMMY(I,J),I=1,IM),J=1,JM)
        CALL EXTRACT_P(DUMMY,Q(1,jsta_2l,L))
      END DO
      DO L = 1,LM
cwas    READ(LRSTRT) ((CWM(I,J,L),I=1,IM),J=1,JM)
        READ(LRSTRT) ((DUMMY(I,J),I=1,IM),J=1,JM)
        CALL EXTRACT_P(DUMMY,CWM(1,jsta_2l,L))
      END DO	
      DO L = 1,LM
cwas    READ(LRSTRT) ((Q2(I,J,L),I=1,IM),J=1,JM)
        READ(LRSTRT) ((DUMMY(I,J),I=1,IM),J=1,JM)
        CALL EXTRACT_P(DUMMY,Q2(1,jsta_2l,L))
      END DO
      DO L = 1,LM+1
cwas    READ(LRSTRT) ((PINT(I,J,L),I=1,IM),J=1,JM)
        READ(LRSTRT) ((DUMMY(I,J),I=1,IM),J=1,JM)
        CALL EXTRACT_P(DUMMY,PINT(1,jsta_2l,L))
      END DO	
      DO L = 1,LM+1
cwas    READ(LRSTRT) ((W(I,J,L),I=1,IM),J=1,JM)
        READ(LRSTRT) ((DUMMY(I,J),I=1,IM),J=1,JM)
        CALL EXTRACT_P(DUMMY,W(1,jsta_2l,L))
      END DO
      DO L = 1,LM
cwas    READ(LRSTRT) ((DWDT(I,J,L),I=1,IM),J=1,JM)
        READ(LRSTRT) ((DUMMY(I,J),I=1,IM),J=1,JM)
        CALL EXTRACT_P(DUMMY,DWDT(1,jsta_2l,L))
      END DO
      DO L = 1,LM
        READ(LRSTRT)
        READ(LRSTRT)
...
cwas    READ(LRSTRT) ((U(I,J,L),I=1,IM),J=1,JM)
        READ(LRSTRT) ((DUMMY(I,J),I=1,IM),J=1,JM)
        CALL EXTRACT_P(DUMMY,U(1,jsta_2l,L))
      END DO	
      DO L = 1,LM	
cwas    READ(LRSTRT) ((V(I,J,L),I=1,IM),J=1,JM)
        READ(LRSTRT) ((DUMMY(I,J),I=1,IM),J=1,JM)
        CALL EXTRACT_P(DUMMY,V(1,jsta_2l,L))
      ENDDO
C***
      IF(ME.EQ.0)THEN
        WRITE(STDOUT,*)'  READ ',LABEL
...
cwas  READ(LRSTRT) AKHS,THS,QS,TWBS,QWBS,HBOT,CFRACL
      ...
      BACKSPACE LRSTRT
C      READ(LRSTRT) DUMMY,DUMMY,DUMMY,DUMMY,DUMMY,DUMMY
C      CALL EXTRACT(DUMMY,HBOT)
C      BACKSPACE LRSTRT
...
CWAS  READ(LRSTRT) U00,UL,LC,SR
      READ(LRSTRT) DUMMY
      CALL EXTRACT_P(DUMMY,U00)
      BACKSPACE LRSTRT
      READ(LRSTRT) DUMMY,UL,DUMMY
      CALL EXTRACT_P(DUMMY,LC)
      BACKSPACE LRSTRT
      READ(LRSTRT) DUMMY,UL,DUMMY,DUMMY
      CALL EXTRACT_P(DUMMY,SR)
C***
      IF(ME.EQ.0)THEN
        WRITE(STDOUT,*)'  READ ',LABEL
      ENDIF
...
C***
      ...
      DO I=1,IM
        ...
        IF(LMHK.EQ.LM)THEN
          ...
          TH10(I,J)=T(I,J,LM)*(1.0E5/(SGML2(LM)*PDSL(I,J)
     +	  +SGML1(LM)*PDTOP+PT))**CAPA
          Q10(I,J)=Q(I,J,LM)
        ENDIF
      ENDDO
      ENDDO
...
C
      DO L=1,LM
         DO J=JSTA,JEND
            DO I=1,IM
               Q(I,J,L) = AMIN1(AMAX1(H1M12,Q(I,J,L)),H99999)
            ENDDO
         ENDDO
      ENDDO    
      IF(ME.EQ.0)THEN
...
!$omp& private(rtopkl)
     ...
     1         (PDTOP*SGML1(L)+PDSL(I,J)*SGML2(L)+PT)
...
!$omp& private(pbi)
      DO L=LM,1,-1
        DO J=JSTA,JEND
        DO I=1,IM
          PBI          =PDSL(I,J)*SG2(L)+PDTOP*SG1(L)+PT
          PINT(I,J,L)  =PBI     
