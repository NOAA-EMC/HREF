...
C     
      ...
      REAL PWTBND(IM,JM,NBND), RHBND(IM,JM,NBND)
C
c     REAL, ALLOCATABLE :: PBINT(IM,JM,NBND+1)
c     REAL, ALLOCATABLE :: QSBND(IM,JM,NBND)
c     REAL, ALLOCATABLE :: PSUM(IM,JM,NBND),PVSUM(IM,JM,NBND),NSUM(IM,JM,NBND)
C
      REAL, ALLOCATABLE :: PBINT(:,:,:),QSBND(:,:,:)
      REAL, ALLOCATABLE :: PSUM(:,:,:),PVSUM(:,:,:),NSUM(:,:,:)
C
      REAL IWL(IM,JM),IWM1
C     
C     INCLUDE COMMONS.
...
!$omp  parallel do
      DO J=JSTA,JEND
      DO I=1,IM
        PBINT(I,J,1)=PDTOP+PD(I,J)+PT
C       PBINT(I,J,1)=PD(I,J)+PT
      ENDDO
      ENDDO
C
      DO LBND=2,NBND+1
