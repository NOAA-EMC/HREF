...
C
      btim = timef()
c      CALL ETAFLD2(IGOUT,JGOUT)
      IF(SIGMA)THEN
       CALL HYBFLD2(IGOUT,JGOUT)
      ELSE
...
C     COMPUTE/POST FIELDS ON PRESSURE SURFACES.
      ...
      IF(SIGMA)THEN
c       IF(SPLINE)THEN
c        print*,'calling sig2pspline'
c        CALL SIG2PSPLINE(IGOUT,JGOUT)
c       ELSE
C        print*,'calling sig2p'
C        CALL SIG2P(IGOUT,JGOUT)
        print*,'calling HYB2P'
        CALL HYB2P(IGOUT,JGOUT)
c       END IF
      ELSE
