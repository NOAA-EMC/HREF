...
!  !$OMP PRIVATE ( ij )
     DO ij = 1 , num_tiles
	write(0,*) 'reset over J : ', j_start(ij),j_end(ij)
	write(0,*) 'reset over I : ', i_start(ij),i_end(ij)
       DO j=j_start(ij),j_end(ij)
       DO i=i_start(ij),i_end(ij)
         wspd10max(i,j) = 0.
         u10max(i,j) = 0.
...
!  !$OMP PRIVATE ( ij )
   DO ij = 1 , num_tiles
     ...
     DO i=i_start(ij),i_end(ij)
       ...
       depth = ( ( ph(i,k+1,j) + phb(i,k+1,j) ) / g ) -                ( ( ph(i,k  ,j) + phb(i,k  ,j) ) / g )
