...
SUBROUTINE wrf_open_for_write_begin( FileName , Comm_compute, Comm_io, SysDepInfo, &
  ...
  CALL wrf_debug( 500, 'module_io.F: in wrf_open_for_write_begin' )
...
SUBROUTINE wrf_get_next_time ( DataHandle, DateStr, Status )
  ...
  IF ( Hndl .GT. -1 ) THEN
    IF ( multi_files( io_form ) .OR. .NOT. (for_out .AND. use_output_servers()) ) THEN
      SELECT CASE ( use_package(io_form) )
        ...
        CASE ( IO_INTIO   )
	write(0,*) 'in IO_INTIO'
	write(0,*) 'have Hndl, DateStr into ext_int_get_next_time: ', Hndl, DateStr

          IF ( multi_files(io_form) .OR.  wrf_dm_on_monitor() ) CALL ext_int_get_next_time( Hndl, DateStr, Status )
	write(0,*) 'return ext_int_get_next_time with DateStr: ', DateStr
          IF ( .NOT. multi_files(io_form) ) THEN
            CALL wrf_dm_bcast_bytes( Status, 4 )
            len_of_str = LEN(DateStr)
            CALL wrf_dm_bcast_string ( DateStr , len_of_str )
        ...
        CASE DEFAULT
          Status = 0
      END SELECT
	write(0,*) 'past CASE statement'
    ELSE IF ( io_form .GT. 0 .AND. for_out .AND. use_output_servers() ) THEN
	write(0,*) 'call wrf_quilt_get_next_time'
  ...
  ELSE
    Status = WRF_ERR_FATAL_BAD_FILE_STATUS
  ENDIF
	write(0,*) 'end of wrf_get_next_time'
  RETURN
...
if ( trim(VarName) .eq. 'T_BYS' ) write(0,*)"module_io.F",'MemOrd(1:2) ',MemOrd(1:2)
if ( trim(VarName) .eq. 'T_BYS' ) write(0,*)"module_io.F",'bdy_mask(P_YSB) ',bdy_mask( P_YSB)
if ( trim(VarName) .eq. 'T_BYS' ) write(0,*)"module_io.F",'bdy_mask(P_YEB) ',bdy_mask( P_YEB)
if ( trim(VarName) .eq. 'T_BYS' ) write(0,*)"module_io.F",'ips - 1 ',ips-1
if ( trim(VarName) .eq. 'T_BYS' ) write(0,*)"module_io.F",'ipe-ips+1 ', ipe-ips+1
...
if ( trim(VarName) .eq. 'T_BYS' ) write(0,*)"module_io.F",'my_displ ', my_displ
if ( trim(VarName) .eq. 'T_BYS' ) write(0,*)"module_io.F",'my_count ', my_count

          CALL mpi_gather( my_displ, 1, MPI_INTEGER, displs, 1, MPI_INTEGER, collective_root, communicator, ierr )
          CALL mpi_gather( my_count, 1, MPI_INTEGER, counts, 1, MPI_INTEGER, collective_root, communicator, ierr )

if ( trim(VarName) .eq. 'T_BYS' ) write(0,*)"module_io.F",'displs ', displs(1:nproc)
if ( trim(VarName) .eq. 'T_BYS' ) write(0,*)"module_io.F",'counts ', counts(1:nproc)
