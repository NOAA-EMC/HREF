...
CONTAINS
  ...
  SUBROUTINE write_nemsinit(grid, gridin, ndom, do_gwd, max_dom, lnsh)
       ...
       REAL, ALLOCATABLE:: DATA_NEMSIO(:,:), PSFC(:,:)
...
!!! not sure about the local array dimensions here (June 7, 2007)
         ...
         IF (.NOT. ALLOCATED(DATA_NEMSIO)) ALLOCATE( DATA_NEMSIO((IDE-IDS+2)*(JDE-JDS+2),NREC)  )
       ...
       ELSE
         ...
         IF (.NOT. ALLOCATED(DATA_NEMSIO)) ALLOCATE( DATA_NEMSIO((IDE-IDS)*(JDE-JDS),NREC)   )
...
!--- FIS ----------------------------------------------------------------
       ...
       if (DOFILL .and. MYPROC .eq. 0) then
         CALL global_fill(temp2,IDS_str,IDE_end,JDS_str,JDE_end,u_var,v_var,e_w_average)
       endif
       ...
       if (MYPROC .eq. 0 ) then
         ...
         nc = nc + 1
         call make_nemsio_array(TEMP2,DATA_NEMSIO(:,nc), IDS_str, IDE_end, JDS_str, JDE_end)
...
!--- STDVTOPO -----------------------------------------------------------------------------------
!mp -  why was this being written twice?
...
!         nc = nc + 1
!         call make_nemsio_array(TEMP2,DATA_NEMSIO(:,nc), IDS_str, IDE_end, JDS_str, JDE_end)
!         recname(nc)='stdh2';reclevtyp(nc)='sfc';reclev(nc)=1
!       endif
...
!         recname(nc)='gwd_orog';reclevtyp(nc)='sfc';reclev(nc)=l
       endif
...
!
       ...
       if (MYPROC .eq. 0 ) then
         ...
         if (nrec /= nc) then
           write(0,*) ' nrec /= nc ',nrec,nc,' check write_nemsinit'
#ifdef _MPI
...
!
         ...
         call nemsio_open(iunit_nemsio,trim(esmf_input_nemsio),gaction='WRITE',iret=iret,  &
                          modelname="NMMB",gdatatype="bin4",idate=IDATE,                   &
!mp                          dimx=IDE_end,dimy=JDE_end,dimz=LM,nframe=NFRAME,                 &
                          dimx=IDE_end-IDS_str+1,dimy=JDE_end-JDS_str+1,dimz=LM,nframe=NFRAME,                 &
                          nsoil=NSOIL,nrec=nrec,                                           &
                          lat=data_nemsio(:,idx_glat), lon=data_nemsio(:,idx_glon),        &
                          dx=dx,dy=dy,                                                     &
                          extrameta=.true.,                                                &
                          nmetavari=nmetavari,                                             &
                          nmetavarr=nmetavarr,                                             &
         ...
         do jrec=1,nrec
!d             call nemsio_getrechead(gfile,jrec,vname,vlevtyp,vlev,iret=iret)
!d             print *,'jrec=',jrec,vname,vlevtyp,vlev,data_nemsio(1:5,jrec)
             call nemsio_writerec(iunit_nemsio,jrec,data_nemsio(:,jrec),iret=iret)
             if (iret/=0) then
  ...
  SUBROUTINE global_fill(array,iglb_s,iglb_e,jglb_s,jglb_e,u_var,v_var,e_w_average)
    IMPLICIT NONE
    INTEGER:: iglb_s,iglb_e,jglb_s,jglb_e, I,J, IND, IADD
    REAL:: array(iglb_s:iglb_e,jglb_s:jglb_e), avg, AS,AN,rcycle
    LOGICAL:: u_var,v_var,e_w_average
...
!       if (J .eq. 10) then
!       write(0,*) 'defined array(1,j) from iglb_e-2: ', iglb_e-2, array(1,j)
!       write(0,*) 'iglb_e-1, array(iglb_e-1,J): ', iglb_e-1, array(iglb_e-1,J)
!       write(0,*) 'array(2,J): ', array(2,J)
!       endif
...
!       endif
      ...
      array(iglb_e,J)=array(3,J)

!       if (J .eq. 10) then
!       write(0,*) 'iglb_e, array(iglb_e,J),array(3,J):', iglb_e, array(iglb_e,J),array(3,J)
!       endif
...
! ---------------------------------------------------------------------
  ...
  SUBROUTINE global_ifill(array,iglb_s,iglb_e,jglb_s,jglb_e,u_var,v_var)
    IMPLICIT NONE
    INTEGER:: iglb_s,iglb_e,jglb_s,jglb_e, I,J,IND,IADD
    INTEGER:: array(iglb_s:iglb_e,jglb_s:jglb_e)
    LOGICAL:: u_var,v_var
...
!
    DO J=jglb_s+1,jglb_e-1
      array(1,J)=array(iglb_e-2,J)
!      array(2,J)=0.5*(array(iglb_e-1,J)+array(2,J))  ! average the duplicated column
      array(iglb_e,J)=array(3,J)
    END DO
...
!
  ...
  END SUBROUTINE global_ifill

! ---------------------------------------------------------------------

  SUBROUTINE global_fill_soil(array,iglb_s,iglb_e,jglb_s,jglb_e,nsoil,u_var,v_var)
    IMPLICIT NONE
    INTEGER:: iglb_s,iglb_e,jglb_s,jglb_e,nsoil,NS,I,J,IND, IADD
    REAL:: array(nsoil,iglb_s:iglb_e,jglb_s:jglb_e)
    LOGICAL:: u_var,v_var
...
!
    DO J=jglb_s+1,jglb_e-1
      array(NS,1,J)=array(NS,iglb_e-2,J)
!      array(NS,2,J)=0.5*(array(NS,iglb_e-1,J)+array(NS,2,J))  ! average the duplicated column
      array(NS,iglb_e,J)=array(NS,3,J)
    END DO
...
!   SOUTH boundary
    ...
    IND=I+IADD
    IF (IND>IGLB_E) IND=IND-IGLB_E+3
         array(NS,i,jglb_s)=array(NS,IND,jglb_s+2)
    END DO
...
!
    ...
    IND=I+IADD
    IF (IND>IGLB_E) IND=IND-IGLB_E+3
         array(NS,i,jglb_e)=array(NS,IND,jglb_e-2)
    END DO

    END DO ! NSOIL loop

  END SUBROUTINE global_fill_soil
