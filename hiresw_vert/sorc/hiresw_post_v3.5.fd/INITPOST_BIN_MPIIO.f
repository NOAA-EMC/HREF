...
! shelter extreme fields
	...
	write(0,*) '(a) nonzero error for mintshltr'
        mintshltr=SPVAL
      else
...
!      ,mpi_status_ignore, ierr)
     ...
     & , mpi_status_ignore, ierr)
        ...
        if (ierr /= 0) then
	write(0,*) 'nonzero error for mintshltr'
          print*,"Error reading ", VarName,"Assigned missing values"
          mintshltr=SPVAL
        end if
      end if
	...
	write(0,*) 'extremes of mintshltr: ', minval(mintshltr),maxval(mintshltr)
...
!      , mpi_status_ignore, ierr)
	...
	write(0,*) 'extremes of maxtshltr: ', minval(maxtshltr),maxval(maxtshltr)
...
!      , mpi_status_ignore, ierr)
	...
	write(0,*) 'extremes of minrhshltr: ', minval(minrhshltr),maxval(minrhshltr)
...
!      , mpi_status_ignore, ierr)
	...
	write(0,*) 'extremes of maxrhshltr: ', minval(maxrhshltr),maxval(maxrhshltr)
...
! WRF ARW outputs 3D cloud cover now
	...
	write(0,*) 'min,max of dum3d_ikj, cfr: ', minval(dum3d_ikj),maxval(dum3d_ikj), &
                                                  minval(cfr),maxval(cfr)
       else
	write(0,*) 'setting cfr to spval: ', spval
        cfr=spval
        cfr=0.
       end if
