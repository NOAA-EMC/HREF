...
c            call print_kpds(nv,2,'_sp',kpds,kens,kprob,xprob)      
            ...
            !maximum
c             kens(4) = 21
c             if(dk5(nv).eq.61) var=prcpmax(:,lv)
c             if(dk5(nv).eq.79) var=snowmax(:,lv)
c             call putgbex(30,jf,kpds,kgds,kens,kprob,xprob,
c     &             kclust,kmembr,lb,var,iret)
c             call print_kpds(nv,2,'_mn',kpds,kens,kprob,xprob)

            !minimum
c             kens(4) = 22
c             if(dk5(nv).eq.61) var=prcpmin(:,lv)
c             if(dk5(nv).eq.79) var=snowmin(:,lv)
c             call putgbex(30,jf,kpds,kgds,kens,kprob,xprob,
c     &             kclust,kmembr,lb,var,iret)
c             call print_kpds(nv,2,'_mn',kpds,kens,kprob,xprob)
2031       continue 
