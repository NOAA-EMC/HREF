...
c raa - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - 
c Rashid Akmaev, May 2013: Correct a bug in calculation of
c     alpha(lons_lat,levs),betta(lons_lat,levs)
c and test various reference pressures (centibars, kiloPascals?)

      real, parameter:: pref0=100.

c In case this doesn't work well, other values to test
c      real, parameter:: pref0=10.
c ...

      real, parameter:: rp0=1./pref0
...
c raa - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - 
c correction1 in alpha and test various pref0
c these are 2x of coefficients in the paper
c          alpha(i,k-1)=(ppi(i,k-1)+ppi(i,k))**xkappa(i,k-1)
c          alpha(i,k-1)=(ppi(i,k)+ppi(i,k+1))**xkappa(i,k)/alpha(i,k-1)

          alpha(i,k-1)=(ppl(i,k-1)*rp0)**xkappa(i,k-1)
          alpha(i,k-1)=(ppl(i,k)*rp0)**xkappa(i,k)/alpha(i,k-1)
        enddo
      enddo
      do k=1,levs-1
        do i=1,lons_lat

c correction in betta and test various pref0
c these are 2x of coefficients in the paper
c          betta(i,k+1)=(ppi(i,k+1)+ppi(i,k+2))**xkappa(i,k+1)
c          betta(i,k+1)=(ppi(i,k  )+ppi(i,k+1))**xkappa(i,k)/betta(i,k+1)

          betta(i,k+1)=(ppl(i,k+1)*rp0)**xkappa(i,k+1)
          betta(i,k+1)=(ppl(i,k)*rp0)**xkappa(i,k)/betta(i,k+1)
c raa - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - 
...
c raa - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - 
c change1 to test various p0 in gamma
c          alnpk = log( ppl(i,k) / 100. )	! cb

          alnpk = log( ppl(i,k)*rp0 )	! cb
...
c change2 to test various p0 in delta
c          alnpk = log( ppl(i,k) / 100. )        ! cb
