...
C     
      RETURN
      END
      SUBROUTINE NETAL_P(FLD3D,ITYPE,L,LMHLMV,EGRID)
C     INCLUDE PARAMETERS.
      INCLUDE "parmeta"
      INCLUDE "CTLBLK.comm"
C
C     DECLARE VARIABLES.
      INTEGER LMHLMV(IM,JM)
      REAL EGRID(IM,JM), FLD3D(IM,jsta_2l:jend_2u,LM)
C
C
C***************************************************************
C     START NETAL HERE.
...
C     LOAD ATMOSPHERIC ETA LAYER L INTO EGRID.
C
C      ELSEIF (ITYPE.GT.1) THEN
CCC THE LINE ABOVE HERE COMMENTED OUT BECAUSE HYBRID MODEL
CCC OUTPUT ALWAYS USES LEVEL ID 109 WHICH COUNTS FROM BOTTOM UP
         LOFF = L-1
         DO J=JSTA,JEND
         DO I=1,IM
            LGET = LMHLMV(I,J) - LOFF
