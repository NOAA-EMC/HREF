...
!$omp&         sqs10k,tbtk,tpspk,tqk,tthbtk,tthesk,tthk)
       ...
       DO 10 I=2,IM-1
        LMHK   =LMH(I,J)
        PSFCK  =PDTOP*SGML1(LMHK)+SGML2(LMHK)*PDSL(I,J)+PT
          PKL = PDTOP*SGML1(KB)+SGML2(KB)*PDSL(I,J)+PT
          IF (ITYPE.EQ.1.AND.(PKL.LT.PSFCK-DPBND.OR.PKL.GT.PSFCK))
     &             GOTO 10
          IF (ITYPE.EQ.1) THEN
            TBTK   =T(I,J,KB)
            QBTK   =Q(I,J,KB)
            APEBTK =(H10E5/(PDTOP*SGML1(KB)+PDSL(I,J)*SGML2(KB)+PT))
     &	    **CAPA
          ELSE
            PKL    =P1D(I,J)
            TBTK   =T1D(I,J)
...
!$omp& private(pkl)
        DO J=JSTA_M,JEND_M
        DO I=2,IM-1
          PKL = PDTOP*SGML1(L)+SGML2(L)*PDSL(I,J)+PT
...
C--------------SCALING PRESSURE & TT TABLE INDEX------------------------
      ...
      DO I=2,IM-1
        KLRES(I,J)=0
        KHRES(I,J)=0
        PKL=SGML1(L)*PDTOP+SGML2(L)*PDSL(I,J)+PT
...
C**
      IF(KNUML.GT.0)THEN
        CALL TTBLEX(TPAR(1,JSTA_2L,L),TTBL,ITB,JTB,KLRES,PDSL,SGML1(L)
     1, SGML2(L),PDTOP,HTM(1,JSTA_2L,L),PT,PL,QQ,PP,RDP,THE0,STHE,RDTHE
     2, THESP,IPTB,ITHTB)
      ENDIF
C***
C***  COMPUTE PARCEL TEMPERATURE ALONG MOIST ADIABAT FOR PRESSURE>PLQ
C**
      IF(KNUMH.GT.0)THEN
        CALL TTBLEX(TPAR(1,JSTA_2L,L),TTBLQ,ITBQ,JTBQ,KHRES,PDSL
     1,SGML1(L),SGML2(L),PDTOP,HTM(1,JSTA_2L,L),PT,PLQ,QQ,PP,RDPQ
...
!$omp& private(dzkl,presk,thetaa,thetap)
        ...
        DO I=2,IM-1
          IF(IDX(I,J).GT.0)THEN
            PRESK=SGML1(L)*PDTOP+SGML2(L)*PDSL(I,J)+PT
            DZKL=T(I,J,L)*(Q(I,J,L)*D608+H1)
     *           *ROG*(PDTOP*DSG1(L)+PDSL(I,J)*DSG2(L))/PRESK
