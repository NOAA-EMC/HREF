...
C
      DO J=JSTA,JEND
         DO I=1,IM
            QBAR(I,J) = AMIN1(AMAX1(H1M12,QBAR(I,J)),H99999)
            QSFC(I,J) = AMIN1(AMAX1(H1M12,QSFC(I,J)),H99999)
         ENDDO
      ENDDO
C     
C     LOOP OVER HORIZONTAL GRID.
...
c         IF (LLMH.LT.LM) SLP(I,J) = PSFC*EXP(ZSFC/TAUAVG)
...
C
      ...
      IEND=IM-1-MOD(J+1,2)
      DO I=2,IEND
        SLP(I,J)=1.5*SLP(I,J+2)-0.5*SLP(I,J+4)
      ENDDO
      ENDDO
      END IF
      END IF
...
C
      DO J=JSTA,JEND
        SLP(1,J)=1.5*SLP(2,J)-0.5*SLP(3,J)
      ENDDO
      DO J=JSTA,JEND
        I=IM-MOD(J+1,2)
        SLP(I,J)=1.5*SLP(I-1,J)-0.5*SLP(I-2,J)
      ENDDO
!$omp  parallel do
      DO J=JSTA,JEND
      DO I=1,IM
