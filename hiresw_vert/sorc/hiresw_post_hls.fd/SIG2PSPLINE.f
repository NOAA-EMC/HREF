...
C     *  10-2001  H CHUANG - MODIFIED TO PROCESS HYBRID MODEL OUTPUT   *
C     *              OPERATIONAL POST                                  *
...
c
      ...
      INCLUDE "params"    
!-----------------------------------------------------------------------
      parameter(nsmud=100,lp2=lm+2,LSMP2=LSM+2)
...
c
      real IWU,IWL,icecl(lm+1),icelsl(lsmp2),icell(im,jm)
     &,iceprs(im,jm,lsmp2) 
      REAL,ALLOCATABLE :: IW(:,:,:)
...
c          alp1u=alog(pint(i,j,l))   ! for nonhydrostatic version
          alp1u=alog(PDTOP*SG1(L)+SG2(l)*pdp+pt)
          alp2u=(alog(SG1(L)*PDTOP+SG2(l)*pdp+pt))**2
c
          dpd=(sg2(l+1)-sg2(l))*pdp+(sg1(l+1)-sg1(l))*pdtop
c
...
ccc level above ground
       do 346 j=jsta,jend
        ...
        do 346 i=ihl,ihh 
         do 347 l=1,lsmp2
	  gar1=sgml1(lm)*pdtop+sgml2(lm)*pdsl(i,j)+pt
	  gar2=exp(ztsl(l)**0.5)
...
C***  REMOVE THE FOLLOWING LINES BECAUSE PSLP and SLP WERE REALDY OUTPUT IN ETAFLD2.f
...
!$omp  parallel do
            ...
            ENDDO
C
            CALL CALRH2(EGRID2,TPRS(1,1,LP),QPRS(1,1,LP),ICEPRS(1,1,LP)
     1,                EGRID1,IM,JM)
