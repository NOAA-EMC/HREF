...
C   01-02-04  H CHUANG - REMOVE LOWER AND UPPER BOUND FOR SFC HEIGHT 
C     
...
C  REMOVE LOWER AND UPPER BOUND FOR SFC HEIGHT BECAUSE THERE ARE
C  NEGATIVE SFC HEIGHT IN CA AT 4 KM RESOLUTION
C            CALL BOUND(GRID1,D00,H99999,IMOUT,JMOUT)
            ID(1:25) = 0
...
C     INSTANTANEOUS PRECIPITATION TYPE.
         IF (IGET(160).GT.0) THEN
            CALL CALWXT(T,Q,RES,PD,HTM,LMH,PREC,PT,PDTOP
     +	    ,SGML1,SGML2,SG1,SG2,IWX1)
            ID(1:25) = 0
C     
