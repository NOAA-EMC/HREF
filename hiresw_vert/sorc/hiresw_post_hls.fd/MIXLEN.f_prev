      SUBROUTINE MIXLEN
     I (ZINT,T,PDSL,PDTOP,SGML1,SGML2,PT,Q2,HGT,HTM,EL0
     I, LM,LM1,LP1,IM,JM
     O, EL)
C
...
C     ------------------------------------------------------------------
C
      DIMENSION ZINT(IM,jsta_2l:jend_2u,LP1)
     &,         EL(IM,jsta_2l:jend_2u,LM),EL0(IM,JM)
     &,         PDSL(IM,JM)
     &,         HTM(IM,jsta_2l:jend_2u,LM),HGT(IM,jsta_2l:jend_2u)
     &,         Q2(IM,jsta_2l:jend_2u,LM)
     &,         T(IM,jsta_2l:jend_2u,LM),APE(IM,JM,2),SGML1(LM)
     &,         SGML2(LM)
       INCLUDE "CTLBLK.comm"
C***********************************************************************
C
...
C       (ASSUME PBL TOP IS AT CPBLT*EL0(K));
      DO J=JSTA_M,JEND_M
      DO I=2,IM-1
        APE(I,J,1)=(1.E5/(PDTOP*SGML1(1)+PDSL(I,J)*SGML2(1)+PT))**CAPA
      ENDDO
      ENDDO
C
      DO 380 L=1,LM1
...
!$omp& private(elst,elvgd,ensq,q2kl,ziag)
      DO J=JSTA_M,JEND_M
      DO I=2,IM-1
        APE(I,J,2)=(1.E5/(PDTOP*SGML1(L+1)+PDSL(I,J)*SGML2(L+1)
     1	+PT))**CAPA
        ENSQ=HTM(I,J,L+1)*
