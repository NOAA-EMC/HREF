...
write(0,*)'after alloc_and_configure_domain ',associated(nested_grid%intermediate_grid)
   ...
   !  Adjust the time step on the clock so that its the same as the history interval
...
!     CALL wrf_get_dom_ti_integer ( in_id , JULDAY , julday , 1 , icnt , ierr ) 
  ...
  parent_grid%em_fnm    = nested_grid%em_fnm
  parent_grid%em_fnp    = nested_grid%em_fnp
  parent_grid%em_rdnw   = nested_grid%em_rdnw
  parent_grid%em_rdn    = nested_grid%em_rdn
  parent_grid%em_dnw    = nested_grid%em_dnw
  parent_grid%em_dn     = nested_grid%em_dn 
  parent_grid%em_znu    = nested_grid%em_znu
  parent_grid%em_znw    = nested_grid%em_znw
         ...
         DO i = its, MIN(ide-1,ite)
            IF ( tsk(i,j) .LT. 170 .or. tsk(i,j) .GT. 400. ) THEN
               ...
               if(tmn(i,j).gt.170. .and. tmn(i,j).lt.400.)then
                  ...
                  CALL wrf_error_fatal3 ( "nup_em.b" , 804 ,  'TSK unreasonable' )
               end if
            END IF
         END DO
      END DO
         ...
         DO i = its, MIN(ide-1,ite)
            IF ( ( ( tmn(i,j) .LT. 170. ) .OR. ( tmn(i,j) .GT. 400. ) ) .AND. ( landmask(i,j) .GT. 0.5 ) ) THEN
               ...
               if(tsk(i,j).gt.170. .and. tsk(i,j).lt.400.)then
                  ...
                  CALL wrf_error_fatal3 ( "nup_em.b" , 824 ,  'TMN unreasonable' )
               endif
            END IF
         END DO
      END DO
         ...
         DO i = its, MIN(ide-1,ite)
            IF ( ( ( tslb(i,1,j) .LT. 170. ) .OR. ( tslb(i,1,j) .GT. 400. ) ) .AND. ( landmask(i,j) .GT. 0.5 ) ) THEN
                  ...
                  if(tsk(i,j).gt.170. .and. tsk(i,j).lt.400.)then
                     ...
                     CALL wrf_error_fatal3 ( "nup_em.b" , 860 ,  'TSLB unreasonable' )
                  endif
            END IF
         END DO
      END DO
...
oops2=oops2+1
               ...
               ELSE
                  ...
                  print *,'tslb=',tslb(i,:,j)
                  print *,'sst=',sst(i,j)
                  CALL wrf_error_fatal3 ( "nup_em.b" , 895 ,  'mismatch_landmask_ivgtyp' )
               END IF
            END IF
         END DO
      END DO
