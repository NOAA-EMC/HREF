     ...
     lwrad_select: SELECT CASE(lw_physics)
        ...
        CASE (GFDLLWSCHEME)
             ...
             IF ( PRESENT(F_QV) .AND. PRESENT(F_QC) .AND.                     &
               ...
               ELSE
                 CALL wrf_error_fatal3 ( "module_radiation_driver.b" , 646 , 'Can not call ETARA (1a). Missing moisture fields.')
               ENDIF
             ELSE
               CALL wrf_error_fatal3 ( "module_radiation_driver.b" , 649 , 'Can not call ETARA (1b). Missing moisture fields.')
             ENDIF
        CASE (CAMLWSCHEME)
...
'set paerlev=29, levsiz=59, cam_abs_dim1=4, and cam_abs_dim2=number of levels (e_vert) in physics namelist for CAM radiation'
               CALL wrf_error_fatal3 ( "module_radiation_driver.b" , 657 ,  wrf_err_message )
             ENDIF
             ...
             ELSE
                CALL wrf_error_fatal3 ( "module_radiation_driver.b" , 703 ,  'arguments not present for calling cam radiation' )
             ENDIF
        CASE DEFAULT
  
             WRITE( wrf_err_message , * ) 'The longwave option does not exist: lw_physics = ', lw_physics
             CALL wrf_error_fatal3 ( "module_radiation_driver.b" , 708 ,  wrf_err_message )
      ...
      ENDDO
        ...
        CASE (GFDLSWSCHEME)
             ...
             IF ( PRESENT(F_QV) .AND. PRESENT(F_QC) .AND.                     &
               ...
               ELSE
                 CALL wrf_error_fatal3 ( "module_radiation_driver.b" , 836 , 'Can not call ETARA (2a). Missing moisture fields.')
               ENDIF
             ELSE
               CALL wrf_error_fatal3 ( "module_radiation_driver.b" , 839 , 'Can not call ETARA (2b). Missing moisture fields.')
             ENDIF

        CASE DEFAULT

             WRITE( wrf_err_message , * ) 'The shortwave option does not exist: sw_physics = ', sw_physics
             CALL wrf_error_fatal3 ( "module_radiation_driver.b" , 845 ,  wrf_err_message )
