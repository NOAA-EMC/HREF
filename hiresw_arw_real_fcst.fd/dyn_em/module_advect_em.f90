...
!  y flux-divergence into tendency
  ...
  ELSE IF ( horz_order == 2 ) THEN
   ...
   ELSE
      ...
      CALL wrf_error_fatal3 ( "module_advect_em.b" , 1122 ,  TRIM( wrf_err_message ) )
...
!  (these are the computations that dont require cb)
...
!   IF ( config_flags%open_xe ) i_end   = MIN(ide-1,ite)
   ...
   ELSE
      ...
      CALL wrf_error_fatal3 ( "module_advect_em.b" , 1402 ,  TRIM( wrf_err_message ) )
...
!  specified uses upstream normal wind at boundaries
  ...
  ELSE
      ...
      CALL wrf_error_fatal3 ( "module_advect_em.b" , 2412 ,  TRIM( wrf_err_message ) )
...
!-------------------- vertical advection
   ...
   ELSE
      ...
      CALL wrf_error_fatal3 ( "module_advect_em.b" , 2710 ,  TRIM( wrf_err_message ) )
...
!  y flux-divergence into tendency
  ...
  ENDDO
   ...
   ELSE
      ...
      CALL wrf_error_fatal3 ( "module_advect_em.b" , 3629 ,  TRIM( wrf_err_message ) )
...
!-------------------- vertical advection
  ...
  ENDDO
   ...
   ELSE
      ...
      CALL wrf_error_fatal3 ( "module_advect_em.b" , 3882 ,  wrf_err_message )
...
ELSE IF (horz_order == 2 ) THEN
   ...
   ELSE
      ...
      CALL wrf_error_fatal3 ( "module_advect_em.b" , 5023 ,  wrf_err_message )
...
! pick up flux contribution for w at the lid, wcs. 13 march 2004
  ...
  ENDDO
   ...
   ELSE
      ...
      CALL wrf_error_fatal3 ( "module_advect_em.b" , 5394 ,  wrf_err_message )
...
!  compute fluxes, 6th order
      ...
      ELSE IF ( j == jde-2 ) THEN  ! 4th order flux 2 in from north boundary
...
!--- end of 3rd order horizontal flux calculation
   ...
   IF( config_flags%periodic_x   .or. &
       config_flags%symmetric_xs .or. &
       (its > ids)                ) degrade_xs = .false.
   IF( config_flags%periodic_x   .or. &
       config_flags%symmetric_xe .or. &
       (ite < ide-1)                ) degrade_xe = .false.
   IF( config_flags%periodic_y   .or. &
       config_flags%symmetric_ys .or. &
       (jts > jds)                ) degrade_ys = .false.
   IF( config_flags%periodic_y   .or. &
       config_flags%symmetric_ye .or. &
       (jte < jde-1)                ) degrade_ye = .false.
...
!--- end of 3nd order horizontal flux calculation

   ELSE

      WRITE ( wrf_err_message , * ) 'module_advect: advect_scalar_pd, h_order not known ',horz_order
      CALL wrf_error_fatal3 ( "module_advect_em.b" , 6545 ,  TRIM( wrf_err_message ) )
...
!-- loop bounds for open or specified conditions
   ...
   ELSE
      ...
      CALL wrf_error_fatal3 ( "module_advect_em.b" , 6918 ,  wrf_err_message )
...
!-- here is the limiter...
   ...
   DO i=i_start, i_end
     ...
     ph_low = (mub(i,j)+mu_old(i,j))*field_old(i,k,j)                   &
                - dt*( msft(i,j)*( rdx*(fqxl(i+1,k,j)-fqxl(i,k,j))      &
                                  +rdy*(fqyl(i,k,j+1)-fqyl(i,k,j)) )    &
                              +rdzw(k)*(fqzl(i,k+1,j)-fqzl(i,k,j))   )  

     flux_out = dt*(msft(i,j)*( rdx*(  max(0.,fqx (i+1,k,j))      &
                                      -min(0.,fqx (i  ,k,j)) )    &
                               +rdy*(  max(0.,fqy (i,k,j+1))      &
                                      -min(0.,fqy (i,k,j  )) ) )  &
                           +rdzw(k)*(  min(0.,fqz (i,k+1,j))      &
                                      -max(0.,fqz (i,k  ,j)) )   )
...
!
  ...
  DO i = i_start, i_end
     ...
     tendency (i,k,j) = tendency(i,k,j)                           &
                - msft(i,j)*( rdx*( fqx (i+1,k,j)-fqx (i,k,j)     &
...
!
  ...
  DO i = i_start, i_end
     ...
     tendency (i,k,j) = tendency(i,k,j)                           &
                - msft(i,j)*( rdy*( fqy (i,k,j+1)-fqy (i,k,j)     &
