    ...
    km_coef: SELECT CASE( config_flags%km_opt )
      ...
      CASE (4)  
            CALL smag2d_km(    config_flags, xkmh, xkmhd, xkmv,         &
                               ...
                               its, ite, jts, jte, kts, kte             )
      CASE DEFAULT
            CALL wrf_error_fatal3 ( "module_diffusion_em.b" , 991 ,  'Please choose diffusion coefficient scheme' )
...
! tke_seed if the drag and flux are off.
    ...
    ELSE
      ...
      DO i = i_start, i_end
        ...
        xkhh(i,k,j)  = MIN( kh_tke_upper_bound*pr_inv, xkmh(i,k,j) * pr_inv )
        xkhv(i,k,j)  = MIN( kv_tke_upper_bound*pr_inv, xkmv(i,k,j) * pr_inv )
      END DO
      END DO
      END DO
    END IF
...
!-----------------------------------------------------------------------
   ...
   ! move out of the conditional, below; this one is needed for 
...
! MARTA: change in the computation of the tkes tendency  at the surface.
...
! WCS 040331
   ...
   heat_flux = config_flags%tke_heat_flux  ! constant heat flux value
                                           ! set in namelist.input
 IF(abs(heat_flux).lt.1.0e-6)THEN
   K=KTS
   DO j = j_start, j_end
   DO i = i_start, i_end
      tendency(i,k,j)= tendency(i,k,j) - &
...
! Apply Deardorffs (1980) "wall effect" at the bottom of the domain. 
...
! Compute z at w points for rdz and rdzw computations.  Well switch z
...
! Now compute zx and zy; well assume that the halo for ph and phb is
! properly filled.
