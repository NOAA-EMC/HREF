...
! end of kludge: 20040604
  ...
  ELSE  ! restart
     ...
     IF ( ierr .NE. 0 ) THEN
       ...
       CALL wrf_error_fatal3 ( "mediation_integrate.b" , 531 ,  message )
     ENDIF
     ...
     nest%imask_xystag = 1
     CALL start_domain ( nest , .TRUE. )
! this doesnt need to be done for moving nests, since ht_coarse is part of the restart
     parent%ht_coarse = parent%ht
...
SUBROUTINE med_hist_out ( grid , stream, config_flags )
  ...
  ! Local
   ...
   IF ( stream .LT. 0 .OR. stream .GT. 11 ) THEN
     ...
     CALL wrf_error_fatal3 ( "mediation_integrate.b" , 1137 ,  message )
   ENDIF
...
!tst
!
! MEP - found that the fcstdone file here on Task 0 
!       typically written PRIOR to completion of the
!       wrfout file, and PRIOR to the creation of the
!       fcstdone file within the quilt.  Eliminate
!       this call to prevent the wrfpost from prematurely
!       trying to read the wrfout file
!
!     call write_fcstdone(TRIM(timestr))
!tst
     END IF
   ELSE
...
SUBROUTINE med_auxinput_in ( grid , stream, config_flags )
  ...
  ! Local
   ...
   IF ( stream .LT. 1 .OR. stream .GT. 11 ) THEN
     ...
     CALL wrf_error_fatal3 ( "mediation_integrate.b" , 1525 ,  message )
   ENDIF
   CALL domain_clock_get( grid, current_time=CT, start_time=ST, current_timestr=timestr )
   SELECT CASE( stream )
     CASE ( 1 )
...
SUBROUTINE med_filter_out ( grid , config_flags )
  ...
  ! Arguments
   ...
   END IF
     ...
     IF ( ierr .NE. 0 ) THEN
       CALL wrf_error_fatal3 ( "mediation_integrate.b" , 1708 ,  message )
     ENDIF
...
! typically a <date> wouldnt be part of the bdy_inname, so just pass a dummy
       ...
       IF ( .NOT. lbc_opened ) THEN
         ...
         CALL open_r_dataset ( head_grid%lbc_fid, TRIM(bdyname) , grid , config_flags , "DATASET=BOUNDARY", ierr )
          IF ( ierr .NE. 0 ) THEN
            ...
            CALL wrf_error_fatal3 ( "mediation_integrate.b" , 1787 ,  message )
          ENDIF
       ELSE
       ...
       IF ( ierr .NE. 0 .and. ierr .NE. WRF_WARN_NETCDF ) THEN
         ...
         CALL wrf_error_fatal3 ( "mediation_integrate.b" , 1804 ,  message )
       ENDIF
