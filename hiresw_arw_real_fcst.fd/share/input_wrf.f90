...
print *,'dx_compare,dy_compare = ',dx_compare,dy_compare
          CALL wrf_error_fatal3 ( "input_wrf.b" , 155 ,  'DX and DY do not match from the namelist and the input file' )
       END IF
    END IF
...
! a hack for sure
          ...
          CALL wrf_error_fatal3 ( "input_wrf.b" , 392 ,  wrf_err_message )
       ENDIF
    ...
    ELSE IF ( switch .EQ. aux_model_input1_only ) THEN
       ...
       IF ( ( ide                             .NE. ide_compare ) .OR. &
         ...
         CALL wrf_error_fatal3 ( "input_wrf.b" , 411 ,  wrf_err_message )
       ENDIF
    ENDIF
...
!
    ...
    IF ( ierr .NE. 0 .AND. ierr .NE. WRF_WARN_NOTSUPPORTED .AND. ierr .NE. WRF_WARN_DRYRUN_READ ) THEN 
      ...
      ELSE
        ...
        CALL wrf_error_fatal3 ( "input_wrf.b" , 491 ,  TRIM(wrf_err_message) )
      ENDIF
