...
!
   ...
   IF ( wrf_dm_on_monitor() ) THEN  
     ...
     ELSE IF( guv < 0.0 ) THEN
       CALL wrf_error_fatal3 ( "module_fdda_psufddagd.b" , 497 , 'In grid FDDA, Guv must be positive.')
     ELSE 
       WRITE(message,'(a,i1,a,e12.4)') &
     ...
     IF( gt > 0.0 ) THEN
       ...
       CALL wrf_message(TRIM(message))
     ELSE IF( gt < 0.0 ) THEN
       CALL wrf_error_fatal3 ( "module_fdda_psufddagd.b" , 509 , 'In grid FDDA, Gt must be positive.')
     ELSE 
       WRITE(message,'(a,i1,a,e12.4)') &
     ...
     IF( gq > 0.0 ) THEN
       ...
       CALL wrf_message(TRIM(message))
     ELSE IF( gq < 0.0 ) THEN
       CALL wrf_error_fatal3 ( "module_fdda_psufddagd.b" , 521 , 'In grid FDDA, Gq must be positive.')
     ELSE
       WRITE(message,'(a,i1,a,e12.4)') &
