...
!-------------------------------------------------------------------          
   SUBROUTINE MRF(U3D,V3D,TH3D,T3D,QV3D,QC3D,QI3D,P3D,PI3D,        &
                  ...
                  dz8w,z,XLV,RV,PSFC,       &
                  ZNT,UST,ZOL,HOL,PBL,REGIME,PSIM,PSIH,            &
...
!
   DO J=jts,jte
      ...
      CALL MRF2D(J,U3D(ims,kms,j),V3D(ims,kms,j),T3D(ims,kms,j),    &
               ...
               HOL(ims,j),PBL(ims,j),REGIME(ims,j),PSIM(ims,j),     &
...
!                                                                                
      DO 420 I=its,ite
        WSPD(I)=SQRT(UX(I,KL)*UX(I,KL)+VX(I,KL)*VX(I,KL))+1.E-9                  
  420 CONTINUE                                                                   
!                                                                                
...
!     COMPUTE TRIDIAGONAL MATRIX ELEMENTS FOR MOMENTUM                           
      ...
      DO I=its,ite
        AD(I,1)=1.                                                               
        A1(I,1)=UX(I,KL)-UX(I,KL)/WSPD(I)*UST(I)*UST(I)/ZQ(I,KL)*DT4         
        A2(I,1)=VX(I,KL)-VX(I,KL)/WSPD(I)*UST(I)*UST(I)/ZQ(I,KL)*DT4         
      ENDDO                                                                      
!                                                                                
      DO K=kte,kts+1,-1
        KK=kme-K
