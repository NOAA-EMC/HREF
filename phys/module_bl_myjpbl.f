...
!	if (I .eq. 165 .and. J .ge. 487 .and. J .lt. 497) then
!	do K=KTE,KTE-2,-1
!	write(6,*) I,K(dyn),J,Q2 into: , I,KTE+1-K,J,Q2K(K)
!	enddo
!	endif
...
!	if (I .eq. 165 .and. J .ge. 487 .and. J .lt. 497) then
!	do K=KTE,KTE-2,-1
!	write(6,*) I,K(dyn),J,Q2 post PRODQ2: , I,KTE+1-K,J,Q2K(K)
!	enddo
!	endif
!
!----------------------------------------------------------------------
...
!***
          CALL DIFCOF(LMH,LPBL(I,J),GM,GH,EL,TK,Q2K,ZHK,AKMK,AKHK      &
                     ,IDS,IDE,JDS,JDE,KDS,KDE                          &
                     ,IMS,IME,JMS,JME,KMS,KME                          &
                     ,ITS,ITE,JTS,JTE,KTS,KTE)
!
...
!	if (I .eq. 165 .and. J .ge. 487 .and. J .lt. 497) then
!	do K=KTE,KTE-2,-1
!	write(6,*) I,K(dyn),J,Q2 post VDIFQ: , I,KTE+1-K,J,Q2K(K)
!	enddo
!	endif
!
!***  SAVE THE NEW TKE
...
!
      REAL,DIMENSION(KTS:KTE-1) :: DTH,ELM
!
!----------------------------------------------------------------------
!**********************************************************************
!----------------------------------------------------------------------
...
!
	...
	if (K .eq. LMH-1 .and. Q2(K) .gt. 1000.) then
	write(6,*) 'big Q2 in PRODQ2: ', EL(K),Q2(K)
	endif
...
!
	...
	if (Q2(LMH) .gt. 1000.) then
	write(6,*) 'big Q2(LMH), USTAR: ', Q2(LMH), USTAR
	endif
!----------------------------------------------------------------------
!
...
!   *                                                                *
!   ******************************************************************
      (LMH,LPBL,GM,GH,EL,T,Q2,Z,AKM,AKH                                &
      ,IDS,IDE,JDS,JDE,KDS,KDE                                         &
      ,IMS,IME,JMS,JME,KMS,KME                                         &
      ,ITS,ITE,JTS,JTE,KTS,KTE)
!----------------------------------------------------------------------
...
!
      REAL :: ADEN,AKQS,BDEN,BESH,BESM,CDEN,CF,DTOZS,ELL,ELOQ2,ELOQ4   &
             ,ELQDZ,ESH,ESM,ESQHF,GHL,GML,Q1L,RDEN,RDZ,Q2before
!
...
!
	...
	if (Q2(LMH-1) .gt. 800.) then
	write(6,*) 'big Q2 in VDIFQ: ', Q2(LMH-1) 
	write(6,*) 'Q2 before recompute:  ', Q2before
	write(6,*) 'DTOZS,AKQS,Q2(LMH) : ', DTOZS,AKQS,Q2(LMH)
	write(6,*) 'CF, CR(LMH-2),RSQ2(LMH-2): ', CF, CR(LMH-2),RSQ2(LMH-2)
	endif
!
      DO K=LMH-2,KTS,-1
