    ...
    SELECT CASE ( use_package( io_form ) )
      CASE ( IO_NETCDF   )
        ...
        IF ( .NOT. multi_files(io_form) ) THEN
          ...
          len_of_str = LEN(Data)
          CALL wrf_dm_bcast_bytes( len_of_str, 4 )
...
SUBROUTINE wrf_open_for_read ( FileName , Comm_compute, Comm_io, SysDepInfo, &
  ...
  ELSE IF ( DataSet .eq. 'BOUNDARY' ) THEN
    CALL get_io_form_boundary( io_form )
	write(0,*) 'from get_io_form_boundary with io_form: ', io_form
  ELSE  ! default if nothing is set in SysDepInfo; use history
    CALL get_io_form_history( io_form )
  ENDIF
  ...
  SELECT CASE ( use_package(io_form) )
    CASE ( IO_NETCDF   )
      IF ( multi_files(io_form) .OR.  wrf_dm_on_monitor() ) THEN
        CALL ext_ncd_open_for_read ( FileName , Comm_compute, Comm_io, SysDepInfo, &
                                     Hndl , Status )
      ENDIF
      IF ( .NOT. multi_files(io_form) ) THEN
        CALL wrf_dm_bcast_bytes( Status, 4 )
    ...
    CASE ( IO_INTIO   )
      IF ( multi_files(io_form) .OR.  wrf_dm_on_monitor() ) THEN
        CALL ext_int_open_for_read ( FileName , Comm_compute, Comm_io, SysDepInfo, &
                                     Hndl , Status )
	write(0,*) 'post ext_int_open_for_read for filename: ', filename
	write(0,*) 'Hndl, Status:', Hndl, Status
      ENDIF
      IF ( .NOT. multi_files(io_form) ) THEN
        CALL wrf_dm_bcast_bytes( Status, 4 )
        CALL wrf_dm_bcast_bytes( Hndl, 4 )
...
!       write(0,*) post add_new_handle Hndl, DataHandle: , Hndl, DataHandle
...
SUBROUTINE wrf_intio_nextrec ( DataHandle , NextRec , Status )
  ...
  CALL wrf_debug( 500, 'module_io.F: in wrf_intio_nextrec' )
...
!	write(0,*) in get_handle DataHandle, Hndl: , DataHandle, Hndl
...
!!!  simply write monitor nodes copy of the data)
