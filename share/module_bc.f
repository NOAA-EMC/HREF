...
!------------
      ...
      IF ((variable_in == 'U') .or. (variable_in == 'V') .or.  &
          (variable_in == 'W') .or. (variable_in == 't')      ) open_bc_copy = .true.
...
!------------
      ...
      IF (variable == 'V') variable = 'v'
      IF (variable == 'W') variable = 'w'
      IF (variable == 'D') variable = 'd'
      IF (variable == 'E') variable = 'e'
      IF (variable == 'F') variable = 'f'
      IF (variable == 'R') variable = 'r'
      IF ((variable_in == 'U') .or. (variable_in == 'V') .or.     &
          (variable_in == 'W') .or. (variable_in == 't') .or.     &
          (variable_in == 'd') .or. (variable_in == 'e') .or. &
          (variable_in == 'f') .or. (variable_in == 'r') ) open_bc_copy = .true.
...
!	write(6,*) spec_bdy_width (3rd arg) is: ,  &
!                     spec_bdy_width, so II cant exceed
    ...
    ELSE IF ( char_stagger .EQ. 'V' ) THEN
       ...
       END DO
!
!	special case for NMM (to avoid staggering business)
!
    ELSE IF ( char_stagger .EQ. 'N' ) THEN

!  NMM (check these loop bounds!)
       DO j = MAX(jds,jts) , MIN(jde,jte)
       DO k = kds , kde 
       DO i = MAX(ids,its) , MIN(ids + spec_bdy_width - 1,ite)
          space_bdy(j,k,i,P_XSB) = data3d(i,k,j)
...
!------------------------------------------------------------------------
 ...
 SUBROUTINE stuff_bdy2( data2d , space_bdy , char_stagger , &
                             ijds , ijde , spec_bdy_width , &
                             ids, ide, jds, jde, kds, kde , &
                             ims, ime, jms, jme, kms, kme , & 
                             its, ite, jts, jte, kts, kte )
 
 !  This routine puts the data in the 2d array into the proper locations
 !  for the lateral boundary arrays.
    ...
    INTEGER , INTENT(IN) :: its, ite, jts, jte, kts, kte
    INTEGER , INTENT(IN) :: ijds , ijde , spec_bdy_width
    REAL , DIMENSION(ims:,jms:) , INTENT(IN) :: data2d
!    REAL , DIMENSION(:,kds:,:,:) , INTENT(OUT) :: space_bdy
    REAL , DIMENSION(ijds:ijde,kds:kde,spec_bdy_width,4) , INTENT(OUT) :: space_bdy
    CHARACTER (LEN=1) , INTENT(IN) :: char_stagger
    ...
    !  X start boundary
                                 ...
                                 IF ( char_stagger .NE. 'N') THEN
 
    DO j = MAX(jds,jts) , MIN(jde-1,jte)
    DO k = kds , kde - 1
    DO i = MAX(ids,its) , MIN(ids + spec_bdy_width - 1,ite)
       space_bdy(j,k,i,P_XSB) = data2d(i,j)
    END DO
    END DO
    END DO
    ...
    DO i = MIN(ide - 1,ite) , MAX(ide - spec_bdy_width,its) , -1
       ii = ide - i
       space_bdy(j,k,ii,P_XEB) = data2d(i,j)
    END DO
    END DO
    END DO
    ...
    DO k = kds , kde - 1
    DO i = MAX(ids,its) , MIN(ide-1,ite)
       space_bdy(i,k,j,P_YSB) = data2d(i,j)
    END DO
    END DO
    END DO
    ...
    DO i = MAX(ids,its) , MIN(ide-1,ite)
       jj = jde - j
       space_bdy(i,k,jj,P_YEB) = data2d(i,j)
    END DO
    END DO
    END DO
 ...
 END SUBROUTINE stuff_bdy2

!------------------------------------------------------------------------
 
 SUBROUTINE stuff_bdytend2 ( data2dnew , data2dold , time_diff , space_bdy , char_stagger , &
                             ijds , ijde , spec_bdy_width      , &
                             ids, ide, jds, jde, kds, kde , &
                             ims, ime, jms, jme, kms, kme , & 
                             its, ite, jts, jte, kts, kte )
 ...
 !  for the lateral boundary arrays.
    ...
    INTEGER , INTENT(IN) :: its, ite, jts, jte, kts, kte
    INTEGER , INTENT(IN) :: ijds , ijde , spec_bdy_width
    REAL , DIMENSION(ims:,jms:) , INTENT(IN) :: data2dnew , data2dold
!    REAL , DIMENSION(:,:,:,:) , INTENT(OUT) :: space_bdy
    REAL , DIMENSION(ijds:ijde,kds:kde,spec_bdy_width,4) , INTENT(OUT) :: space_bdy
    CHARACTER (LEN=1) , INTENT(IN) :: char_stagger
    REAL , INTENT(IN) :: time_diff ! seconds
    ...
    DO k = kds , kde - 1
    DO i = MAX(ids,its) , MIN(ids + spec_bdy_width - 1,ite)
      space_bdy(j,k,i,P_XSB) = ( data2dnew(i,j) - data2dold(i,j) ) / time_diff
!     space_bdy(j,k,i,P_XSB) = 0. ! zeroout
    END DO
    END DO
    END DO
    ...
    DO i = MIN(ide - 1,ite) , MAX(ide - spec_bdy_width,its) , -1
       ii = ide - i
       space_bdy(j,k,ii,P_XEB) = ( data2dnew(i,j) - data2dold(i,j) ) / time_diff
!     space_bdy(j,k,ii,P_XEB) = 0. ! zeroout
    END DO
    END DO
    END DO
    ...
    DO k = kds , kde - 1
    DO i = MAX(ids,its) , MIN(ide-1,ite)
      space_bdy(i,k,j,P_YSB) = ( data2dnew(i,j) - data2dold(i,j) ) / time_diff
!     space_bdy(i,k,j,P_YSB) = 0. ! zeroout
    END DO
    END DO
    END DO
    ...
    DO i = MAX(ids,its) , MIN(ide-1,ite)
       jj = jde - j
       space_bdy(i,k,jj,P_YEB) = ( data2dnew(i,j) - data2dold(i,j) ) / time_diff
!      space_bdy(i,k,jj,P_YEB) = 0. ! zeroout
    END DO
    END DO
    END DO
