...
!.. physics options.
      ...
      real(kind=kind_phys),DIMENSION((ITE-ITS+1),LM,NUM_WATER) :: GR1   
!
...
!
        if (ntoz.le.0) then
          ...
          gr1(1:im,l,1)=max(o3((its+dps-1):(its+dps-1+im-1),j,l),epso3)
        endif
!
        GR1(1:im,L,2)=0.d0
        GR1(1:im,L,3)=CW((its+dps-1):(its+dps-1+im-1),J,L)
...
!   present and RH > 95%
        ...
        DO I=1,IM
          ...
          ESAT=FPVS(T((its+dps-1+I-1),J,L))                   !-- Saturation vapor pressure (kPa)
        ...
        CLDF(1:im,L)=CLDFRA((its+dps-1):(its+dps-1+im-1),J,L)           !CLOUD FRACTION (ICWP==-1)
      ENDDO
!
...
!=================================================================
      ...
      IF (ICWP /= -1) THEN
         ...
         !===========================================================
            DO I=1,IM
              ARG_CW = MAXVAL( CW((its+dps-1+I-1),J,1:LM) )
...
!-- Added a time-averaged convective cloud fraction calculation
            WHERE (CU_cloud) ACFRCV((its+dps-1):(its+dps-1+im-1),J)=ACFRCV((its+dps-1):(its+dps-1+im-1),J)+CLDSA_V(1:im,4)
