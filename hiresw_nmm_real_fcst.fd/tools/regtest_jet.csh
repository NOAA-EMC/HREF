...
set FAIL_MAIL = ( jacquesm@fsl.noaa.gov )
set GOOD_MAIL = ( jacquesm@fsl.noaa.gov )
...
if ( $#argv == 0 ) then
#	echo "Please enter either a date for cvs checkout. ex wrftest.csh -D date"
#	echo " or a file name containing WRF. ex wrftest.csh -f tarfile"
...
if ( $ARCH[1] == OSF1 ) then
	...
	set DEF_DIR		= /p10/acb/users/jacquesm/wrfReg
	set MAIL		= /usr/bin/mailx
	set COMPOPTS		= ( 1 3 )
	set MPIRUNCOMMAND 	= ( mpirun -np 4 )
endif
...
if      ( $acquire_from == "ftp" ) then
	...
	ftp -n ftp.ucar.edu < ftp_script_data
        ...
        #       A tar file of the WRF source was provided, so that is used.
        #       A tar file of the input data was provided, so that is used.
        tar xvf $thefile
        cp ../data.tar.gz .
endif
...
foreach core ( eh_real em_real eh_b_wave eh_quarter_ss em_b_wave em_quarter_ss )
	...
	foreach compopt ( $COMPOPTS )
		...
		if ( $ok != 0 ) then
			...
			if ( ( ( $core == em_real ) || ( $core == eh_real ) ) && ( $compopt == 1 ) ) then
	...
	foreach phys_option ( $PHYSOPTS )
		...
		if ( ( $core == em_real ) || ( $core == eh_real ) ) then
			...
			foreach compopt ( $COMPOPTS )
				...
				if ( ( $phys_option == $PHYSOPTS[1] ) && ( $compopt == 1 ) ) then
				...
				if ( $compopt == 1 ) then
					...
					if ( ( -e wrfinput ) && ( -e wrfbdy ) ) then
						...
						$MAIL -s "WRF FAIL making IC/BC" $FAIL_MAIL < ${DEF_DIR}/wrftest.output
						exit ( 4 )
					endif
				endif
				...
				if ( -e wrfout_01_000000 ) then
					...
					$MAIL -s "WRF FAIL FCST" $FAIL_MAIL < ${DEF_DIR}/wrftest.output
					exit ( 5 )
				endif
				...
				mv wrfout_01_000000 wrfout_01_000000.${phys_option}.$compopt
			...
			foreach compopt ( $COMPOPTS )
				...
				if ( $compopt == 1 ) then
					...
					if ( ( -e wrfinput ) && ( -e wrfbdy ) ) then
						echo "SUMMARY generate IC/BC for $core           parallel $compopt PASS" >>! ${DEF_DIR}/wrftest.output
						echo "-------------------------------------------------------------" >> ${DEF_DIR}/wrftest.output
					else
						echo "SUMMARY generate IC/BC for $core           parallel $compopt FAIL" >>! ${DEF_DIR}/wrftest.output
						$MAIL -s "WRF FAIL making IC/BC" $FAIL_MAIL < ${DEF_DIR}/wrftest.output
						exit ( 4 )
					endif
				endif
				...
				rm wrfout_01_000000.$compopt >& /dev/null
				...
				if ( -e wrfout_01_000000 ) then
					echo "SUMMARY generate FCST  for $core           parallel $compopt PASS" >>! ${DEF_DIR}/wrftest.output
					echo "-------------------------------------------------------------" >> ${DEF_DIR}/wrftest.output
				else
					echo "SUMMARY generate FCST  for $core           parallel $compopt FAIL" >>! ${DEF_DIR}/wrftest.output
					$MAIL -s "WRF FAIL FCST" $FAIL_MAIL < ${DEF_DIR}/wrftest.output
					exit ( 5 )
				endif
		...
		if ( ${#COMPOPTS} != 1 ) then
			...
			set DIFFWRF = ${DEF_DIR}/regression_test/WRFV1/external/io_netcdf/diffwrf
			  ...
			  if ( ( -e wrfout_01_000000.${phys_option}.1 ) && ( -e wrfout_01_000000.${phys_option}.2 ) ) then
				$DIFFWRF wrfout_01_000000.${phys_option}.1 wrfout_01_000000.${phys_option}.2 >& /dev/null
			  else
				touch fort.88 fort.98
			  endif
			  if ( ! -e fort.88 ) then
				echo "SUMMARY serial vs OMP  for $core physics $phys_option parallel $compopt PASS" >>! ${DEF_DIR}/wrftest.output
				echo "-------------------------------------------------------------" >> ${DEF_DIR}/wrftest.output
			  else
				echo "SUMMARY serial vs OMP  for $core physics $phys_option parallel $compopt FAIL" >>! ${DEF_DIR}/wrftest.output
				echo "-------------------------------------------------------------" >> ${DEF_DIR}/wrftest.output
			  endif
			...
			rm fort.88 fort.98 >& /dev/null
			if ( ( -e wrfout_01_000000.${phys_option}.1 ) && ( -e wrfout_01_000000.${phys_option}.3 ) ) then
				$DIFFWRF wrfout_01_000000.${phys_option}.1 wrfout_01_000000.${phys_option}.3 >& /dev/null
			else
				touch fort.88 fort.98
			endif
			if ( ! -e fort.88 ) then
