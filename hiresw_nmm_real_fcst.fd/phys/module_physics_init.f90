...
!=================================================================
   SUBROUTINE phy_init ( id, config_flags, DT, restart, zfull, zhalf,     &
                         ...
                         MASS_FLUX,                              &
                         RTHFTEN, RQVFTEN,                       &
                         CLDFRA,CLDFRA_OLD,GLW,GSW,EMISS,LU_INDEX,&
   ...
   IF ( allowed_to_read ) THEN
      ...
      IF ( wrf_dm_on_monitor() ) THEN
        ...
        IF ( ierr .NE. OPEN_OK ) THEN
          ...
          CALL wrf_error_fatal3 ( "module_physics_init.b" , 578 ,  message ) 
        END IF
      ENDIF
        ...
        DO ls=1,luseas                                   
          ...
          DO LC=1,LUCATS                               
            IF(found_lu)THEN                  
              ...
              IF(LC.NE.LI)CALL wrf_error_fatal3 ( "module_physics_init.b" , 637 ,  'module_start: MISSING LANDUSE UNIT ' )
            ELSE                                                            
              IF ( wrf_dm_on_monitor() ) THEN
                READ (landuse_unit,*)                                                  
              ENDIF
      ...
      DO j = jts, jtf
        DO i = its, itf
          ...
          IF(IS.LT.0.OR.IS.GT.LUN.AND.allowed_to_read)THEN                                        
            ...
            CALL wrf_error_fatal3 ( "module_physics_init.b" , 676 ,  TRIM ( wrf_err_message ) )
          ENDIF                                                            
   ...
   lwrad_select: SELECT CASE(config_flags%ra_lw_physics)
        ...
        CASE (CAMLWSCHEME)
             ...
             CALL camradinit(                                  &
                         ...
                         its, ite, jts, jte, kts, kte      )
             ELSE
                CALL wrf_error_fatal3 ( "module_physics_init.b" , 879 ,  'arguments not present for calling cam radiation' )
             ENDIF
...
!                              num_roof_layers,num_wall_layers,num_road_layers, & !urban
             ...
             ELSE
                CALL wrf_error_fatal3 ( "module_physics_init.b" , 1184 ,  'arguments not present for calling urban model' )
             ENDIF
          ENDIF
