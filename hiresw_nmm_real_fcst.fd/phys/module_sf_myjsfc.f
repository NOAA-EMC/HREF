...
!----------------------------------------------------------------------
      SUBROUTINE MYJSFC(ITIMESTEP,HT,DZ                                & 
                       ...
                       ,AKHS,AKMS                                      &
                       ,CHS,CHS2,HFX,QFX,FLX_LH,FLHC,FLQC,QGH,CPM,CT   &
...
!
          CALL SFCDIF(NTSD,SEAMASK,THSK(I,J),QSFC(I,J),PSFC            &
                     ...
                     ,CHS(I,J),CHS2(I,J),HFX(I,J),QFX(I,J),FLX_LH(I,J) &
...
!----------------------------------------------------------------------
      SUBROUTINE SFCDIF(NTSD,SEAMASK,THS,QS,PSFC                       &
                       ...
                       ,CHS,CHS2,HFX,QFX,FLX_LH,FLHC,FLQC,QGH,CPM      &
                       ,ULOW,VLOW,TLOW,THLOW,THELOW,QLOW,CWMLOW        &
                       ,ZSL,PLOW                                       &
...
!
      REAL,INTENT(OUT) :: CHS,CHS2,FLX_LH,HFX,QFX,FLHC,FLQC,PSHLTR     &
...
!
        ZQ=ZT
!
!***  IF QS FROM LAND SURFACE SCHEME IS A MIXING RATIO
!***  THEN CONVERT TO SPECIFIC HUMIDITY.
!
!!!     QZ0=QS
        QZ0=QS/(1.+QS)
!----------------------------------------------------------------------
...
!!!   QGH=PQ0/PSHLTR*EXP(A2S*(TSK-A3S)/(TSK-A4S))
	...
	if (abs(USTAR) .gt. 10.) then
	write(6,*) 'BIG USTAR: ', USTAR
	endif
!----------------------------------------------------------------------
!
...
!
      INTEGER :: I,IRECV,J,K,ITF,JTF,KTF,MAXGBL_IVGTYP,MAXLOC_IVGTYP   &
     &,          MPI_COMM_COMP
...
!
      ...
      DO I=ITS,ITE
        SM=SEAMASK(I,J)-1.
        IF(SM+XICE(I,J).LT.0.5)THEN
          Z0(I,J)=Z0(I,J)+VZ0TBL_24(IVGTYP(I,J))
        ENDIF
      ENDDO
      ENDDO
