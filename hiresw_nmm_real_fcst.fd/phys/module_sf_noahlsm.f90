...
! initialize three Noah LSM related tables
   ...
   IF ( errflag .EQ. 1 ) THEN
      CALL wrf_error_fatal3 ( "module_sf_noahlsm.b" , 978 ,  "module_sf_noahlsm.F: lsminit: out of range value "// &
...
!                                                                       
       ...
       IF ( wrf_dm_on_monitor() ) THEN
        ...
        IF(ierr .NE. OPEN_OK ) THEN
          ...
          CALL wrf_error_fatal3 ( "module_sf_noahlsm.b" , 1123 ,  message ) 
        END IF
...
!                                                                       
      IF ( wrf_dm_on_monitor() ) THEN
        ...
        IF(ierr .NE. OPEN_OK ) THEN
          ...
          CALL wrf_error_fatal3 ( "module_sf_noahlsm.b" , 1195 ,  message ) 
        END IF
 ...
 2003   CONTINUE                                                        
      ...
      IF(LUMATCH.EQ.0)THEN                                            
          ...
          CALL wrf_error_fatal3 ( "module_sf_noahlsm.b" , 1245 ,  'INCONSISTENT OR MISSING SOILPARM FILE' )
      ENDIF                                                           
...
!                                                                       
      IF ( wrf_dm_on_monitor() ) THEN
        ...
        IF(ierr .NE. OPEN_OK ) THEN
          ...
          CALL wrf_error_fatal3 ( "module_sf_noahlsm.b" , 1256 ,  message ) 
        END IF
...
!   TH2        AIR TEMPERATURE (K) AT HEIGHT 2M ABOVE GROUND         
...
! ----------------------------------------------------------------------         
         ...
         ELSE                                                                    
            SNDENS = SNEQV / SNOWH                                               
            IF(SNDENS > 1.0) THEN                                               
             CALL wrf_error_fatal3 ( "module_sf_noahlsm.b" , 1644 ,  'Physical snow depth is less than snow water equiv.' )
            ENDIF                                                                
...
!                                                                                
               IF (SOILTYP .gt. SLCATS) THEN                                     
                        CALL wrf_error_fatal3 ( "module_sf_noahlsm.b" , 3280 ,  'Warning: too many input soil types' )
               END IF                                                            
               IF (VEGTYP .gt. LUCATS) THEN                                      
                     CALL wrf_error_fatal3 ( "module_sf_noahlsm.b" , 3283 ,  'Warning: too many input landuse types' )
               END IF                                                            
               IF (SLOPETYP .gt. SLPCATS) THEN                                   
                     CALL wrf_error_fatal3 ( "module_sf_noahlsm.b" , 3286 ,  'Warning: too many input slope types' )
               END IF                                                            
      ...
      LAI = LAITBL (VEGTYP)                                                      
               ...
               IF (NROOT .gt. NSOIL) THEN
                  WRITE (err_message,*) 'Error: too many root layers ',  &
                                                 NSOIL,NROOT
                  CALL wrf_error_fatal3 ( "module_sf_noahlsm.b" , 3348 ,  err_message )
! ----------------------------------------------------------------------         
