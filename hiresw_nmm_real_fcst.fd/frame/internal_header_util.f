...
!
  ...
  hdrbuf(2) = int_open_for_read
	write(0,*) 'int_open_for_read: ', int_open_for_read
  i = 3
  hdrbuf(i) = DataHandle     ; i = i+1
...
SUBROUTINE int_gen_td_header_char( hdrbuf, hdrbufsize, itypesize, typesize, &
                              DataHandle, DateStr, Element, Data, Count, code )
  IMPLICIT NONE
  INCLUDE 'intio_tags.h'
  INTEGER, INTENT(INOUT)       ::  hdrbuf(*)
  ...
  INTEGER, INTENT(IN)          ::  DataHandle, code
!Local
  INTEGER i, n, Count, DummyCount, DummyData
!

	write(0,*) 'into int_gen_td_header_char'
	write(0,*) 'call int_gen_ti_header_c'

!!!!!!! fourth argument (1 or typesize) appears important
  CALL int_gen_ti_header_c ( hdrbuf, hdrbufsize, itypesize, 1, &
                           DataHandle, DummyData, DummyCount, code )
	write(0,*) 'in int_gen_td_header_char, hdrbufsize: ', hdrbufsize
  i = hdrbufsize/itypesize + 1 ;
  CALL int_pack_string ( DateStr, hdrbuf( i ), n ) ; i = i + n
  CALL int_pack_string ( Element, hdrbuf( i ), n ) ; i = i + n
  CALL int_pack_string ( Data, hdrbuf( i ), n ) ; i = i + n
	write(0,*) 'in int_gen_td_header_char...'
	write(0,*) 'dummyData: ', DummyData
	write(0,*) 'packed DateStr, Element, Data: ',  DateStr, Element, Data
	write(0,*) 'I= ', I
!  CALL int_pack_string ( VarName, hdrbuf( i ), n ) ; i = i + n
  hdrbufsize = (i-1) * itypesize + hdrbufsize ! return the number in bytes
  hdrbuf(1) = hdrbufsize
...
SUBROUTINE int_get_td_header_char( hdrbuf, hdrbufsize, itypesize, typesize, &
  ...
  INTEGER, INTENT(OUT)          ::  DataHandle, code
!Local
  INTEGER i, n, DummyCount, DummyData, Count
!

  CALL int_get_ti_header_c ( hdrbuf, hdrbufsize, itypesize, typesize, &
                           DataHandle, DummyData, DummyCount, code )
  i = hdrbufsize/itypesize + 1 ;
  CALL int_unpack_string ( DateStr, hdrbuf( i ), n ) ; i = i + n
  CALL int_unpack_string ( Element, hdrbuf( i ), n ) ; i = i + n
