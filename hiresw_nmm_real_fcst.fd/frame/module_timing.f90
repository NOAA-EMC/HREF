...
CONTAINS
   ...
   SUBROUTINE start_timing
      ...
      cn = cn + 1
      IF ( cn .gt. cnmax ) THEN
        CALL wrf_error_fatal3 ( "module_timing.b" , 26 ,  'module_timing: clock nesting error (too many nests)' )
        RETURN
      ENDIF
...
!     CALL CPU_TIME ( cpu_1 )
   ...
   SUBROUTINE end_timing ( string )
      ...
      IF ( cn .lt. 1 ) THEN
        CALL wrf_error_fatal3 ( "module_timing.b" , 42 ,  'module_timing: clock nesting error, cn<1' ) 
      ELSE IF ( cn .gt. cnmax ) THEN
        CALL wrf_error_fatal3 ( "module_timing.b" , 44 ,  'module_timing: clock nesting error, cn>cnmax' ) 
      ENDIF
