...
! Local
    ...
    ! Find the VarName if its in the buffer already
    ii = 1
    found = .false.
      ...
      IF ( TRIM(VarName) .EQ. TRIM(outbuf_table(ii)%VarName) ) THEN
        IF ( TRIM(DateStr) .EQ. TRIM(outbuf_table(ii)%DateStr) ) THEN
          IF ( TRIM(MemoryOrder) .EQ. TRIM(outbuf_table(ii)%MemoryOrder) ) THEN
            found = .true.
          ELSE
            CALL wrf_error_fatal3 ( "module_quilt_outbuf_ops.b" , 341 , "store_patch_in_outbuf: memory order disagreement")
          ENDIF
        ELSE
          CALL wrf_error_fatal3 ( "module_quilt_outbuf_ops.b" , 344 , "store_patch_in_outbuf: multiple dates in buffer")
        ENDIF
      ELSE
        ii = ii + 1
      ENDIF
    ...
    IF ( .NOT. found ) THEN
      ...
      ELSE
        ...
        CALL wrf_error_fatal3 ( "module_quilt_outbuf_ops.b" , 360 , mess)
      ENDIF
...
! Local
    ...
    IF ( ierr .NE. 0 ) CALL wrf_error_fatal3 ( "module_quilt_outbuf_ops.b" , 453 ,  "store_piece_of_field" )
    RETURN
