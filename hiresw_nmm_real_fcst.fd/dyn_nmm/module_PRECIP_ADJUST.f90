...
!***  Remember that in WRF, local and global (i,j) are the same, so dont
...
! POINT TO MISSING (SO WE WONT DO SNOW ADJUSTMENT HERE).
!
!-----------------------------------------------------------------------
!
      IMPLICIT NONE
...
!
 ...
 1010     FORMAT('ADJPPT-CHKSNOW: TIMES, SR=',F6.0,X,F6.4)
          CALL WRF_MESSAGE(MESSAGE)
        ENDIF
      ENDIF
 10   CONTINUE
...
!     
! ABSTRACT:
!     ADJPPT  MAKES ADJUSTMENT TO MODELS TEMPERATURE, MOISTURE, HYDROMETEOR
...
!
      ...
      &    FRACT1,FRACT2
 1010    FORMAT('ADJPPT: NTSD,NTSP,TIMES=',I4,X,I4,1X,F6.0,' IHR1,IHR2=',&
      &    I1,X,I1,' TPHS1,TPHS2=',F6.0,X,F6.0,' FRACT1,FRACT2=',        &
      &    2(X,F6.4))
        CALL WRF_MESSAGE(MESSAGE)
      ENDIF
!
!-----------------------------------------------------------------------
!   FRACT1/2 IS THE FRACTION OF IHR1/2S PRECIP THAT WE WANT FOR
