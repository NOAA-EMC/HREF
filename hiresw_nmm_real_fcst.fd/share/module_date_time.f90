...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
   ...
   SUBROUTINE geth_idts (ndate, odate, idts)
      ...
      IF (.not. npass) THEN
         ...
         CALL wrf_error_fatal3 ( "module_date_time.b" , 313 ,  TRIM ( wrf_err_message ) )
      END IF
      
      IF (.not. opass) THEN
         WRITE( wrf_err_message , * ) 'module_date_time: geth_idts: Bad ODATE: ', odate(1:olen)
         CALL wrf_error_fatal3 ( "module_date_time.b" , 318 ,  TRIM ( wrf_err_message ) )
      END IF
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
   ...
   SUBROUTINE geth_newdate (ndate, odate, idt)
      ...
      IF (.not.opass) THEN
         ...
         CALL wrf_error_fatal3 ( "module_date_time.b" , 523 ,  TRIM ( wrf_err_message ) )
      END IF
      ...
      ELSE
         ...
         CALL wrf_error_fatal3 ( "module_date_time.b" , 569 ,  TRIM ( wrf_err_message ) )
      END IF
...
34 FORMAT (I4.4,1x,I2.2,1x,I2.2,1x,I2.2,1x,I2.2,1x,I2.2,1x,I4.4)
   ...
   SUBROUTINE wrf_timetoa ( time, str )
      ...
      ! Assertion
      IF ( LEN(str) < 19 ) THEN
        CALL wrf_error_fatal3 ( "module_date_time.b" , 799 ,  'wrf_timetoa:  str is too short' )
      ENDIF
      tmpstr = ''
