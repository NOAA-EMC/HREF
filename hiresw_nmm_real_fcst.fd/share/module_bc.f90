...
!  set the bdyzone.  We are hardwiring this here and well
...
!  check the number of conditions for each boundary
   ...
   IF( (xs_bc /= 1) .or. &
     ...
     CALL wrf_error_fatal3 ( "module_bc.b" , 146 ,  ' *** Error in boundary condition specification ' )
...
!  to the calling routine.
   ...
   IF( config_flags%periodic_x   .or. &
       ...
       IF( bzone < bzone_min) THEN  
         ...
         CALL wrf_error_fatal3 ( "module_bc.b" , 173 ,  wrf_err_message )
...
!  set open b.c in X copy into boundary zone here.  WCS, 19 March 2000
        ...
        open_xs: IF( ( config_flags%open_xs   .or. &
            ...
            DO j = MAX(jds,jts-1), MIN(jte+1,jde+jstag)
              ...
              data(ids-2,j) = data(ids,j)
              data(ids-3,j) = data(ids,j)
            ENDDO
        ...
        open_xe: IF( ( config_flags%open_xe   .or. &
          ...
          IF ( variable /= 'u' .and. variable /= 'x') THEN
            ...
            DO j = MAX(jds,jts-1), MIN(jte+1,jde+jstag)
              ...
              data(ide+1,j) = data(ide-1,j) 
              data(ide+2,j) = data(ide-1,j) 
            ENDDO
          ...
          ELSE
            ...
            DO j = MAX(jds,jts-1), MIN(jte+1,jde+jstag)
              ...
              data(ide+2,j) = data(ide,j)
              data(ide+3,j) = data(ide,j)
            ENDDO
...
!  set open b.c in Y copy into boundary zone here.  WCS, 19 March 2000
        ...
        open_ys: IF( ( config_flags%open_ys   .or. &
            ...
            DO i = MAX(ids,its-1), MIN(ite+1,ide+istag)
              ...
              data(i,jds-2) = data(i,jds) 
              data(i,jds-3) = data(i,jds) 
            ENDDO
        ...
        open_ye: IF( ( config_flags%open_ye   .or. &
          ...
          IF  (variable /= 'v' .and. variable /= 'y' ) THEN
            ...
            DO i = MAX(ids,its-1), MIN(ite+1,ide+istag)
              ...
              data(i,jde+1) = data(i,jde-1) 
              data(i,jde+2) = data(i,jde-1) 
            ENDDO                               
          ...
          ELSE
            ...
            DO i = MAX(ids,its-1), MIN(ite+1,ide+istag)
              ...
              data(i,jde+2) = data(i,jde) 
              data(i,jde+3) = data(i,jde) 
            ENDDO                               
...
!  set open b.c in X copy into boundary zone here.  WCS, 19 March 2000
        ...
        open_xs: IF( ( config_flags%open_xs   .or. &
            ...
            DO k = kts, k_end
              ...
              data(ids-2,k,j) = data(ids,k,j)
              data(ids-3,k,j) = data(ids,k,j)
            ENDDO
            ENDDO
        ...
        open_xe: IF( ( config_flags%open_xe   .or. &
          ...
          IF (variable /= 'u' .and. variable /= 'x' ) THEN
            ...
            DO k = kts, k_end
              ...
              data(ide+1,k,j) = data(ide-1,k,j)
              data(ide+2,k,j) = data(ide-1,k,j)
            ENDDO
            ENDDO
...
!!!!!!! I am not sure about this one!  JM 20020402
            ...
            DO k = kts, k_end
              ...
              data(ide+2,k,j) = data(ide,k,j)
              data(ide+3,k,j) = data(ide,k,j)
            ENDDO
            ENDDO
...
!  set open b.c in Y copy into boundary zone here.  WCS, 19 March 2000
        ...
        open_ys: IF( ( config_flags%open_ys   .or. &
            ...
            DO i = MAX(ids,its-1), MIN(ite+1,ide+istag)
              ...
              data(i,k,jds-2) = data(i,k,jds) 
              data(i,k,jds-3) = data(i,k,jds) 
            ENDDO
            ENDDO
        ...
        open_ye: IF( ( config_flags%open_ye   .or. &
          ...
          IF (variable /= 'v' .and. variable /= 'y' ) THEN
            ...
            DO i = MAX(ids,its-1), MIN(ite+1,ide+istag)
              ...
              data(i,k,jde+1) = data(i,k,jde-1) 
              data(i,k,jde+2) = data(i,k,jde-1) 
            ENDDO                               
            ENDDO
          ...
          ELSE
            ...
            DO i = MAX(ids,its-1), MIN(ite+1,ide+istag)
              ...
              data(i,k,jde+2) = data(i,k,jde) 
              data(i,k,jde+3) = data(i,k,jde) 
            ENDDO                               
            ENDDO
          ...
          DO k = kts, ktf
            DO i = max(its,b_limit+ibs), min(itf,ibe-b_limit)
              ...
              fls0 = field_bdy(i, k, b_dist+1, P_YSB) &
                   ...
                   - field(i,k,j)
              fls1 = field_bdy(im1, k, b_dist+1, P_YSB) &
                   + dtbc * field_bdy_tend(im1, k, b_dist+1, P_YSB) &
                   - field(im1,k,j)
              fls2 = field_bdy(ip1, k, b_dist+1, P_YSB) &
                   + dtbc * field_bdy_tend(ip1, k, b_dist+1, P_YSB) &
                   - field(ip1,k,j)
          ...
          DO k = kts, ktf
            DO i = max(its,b_limit+ibs), min(itf,ibe-b_limit)
              ...
              fls0 = field_bdy(i, k, b_dist+1, P_YEB) &
                   ...
                   - field(i,k,j)
              fls1 = field_bdy(im1, k, b_dist+1, P_YEB) &
                   + dtbc * field_bdy_tend(im1, k, b_dist+1, P_YEB) &
                   - field(im1,k,j)
              fls2 = field_bdy(ip1, k, b_dist+1, P_YEB) &
                   + dtbc * field_bdy_tend(ip1, k, b_dist+1, P_YEB) &
                   - field(ip1,k,j)
...
!    REAL , DIMENSION(:,:,:,:) , INTENT(OUT) :: space_bdy
    ...
    IF ( char_stagger .EQ. 'W' ) THEN
       ...
       DO i = MAX(ids,its) , MIN(ids + spec_bdy_width - 1,ite)
          space_bdy(j,k,i,P_XSB,1) = data3d(i,k,j)
       END DO
       END DO
       END DO
    ...
    IF      ( char_stagger .EQ. 'U' ) THEN
       ...
       DO i = MIN(ide,ite) , MAX(ide - spec_bdy_width + 1,its) , -1
          ii = ide - i + 1
          space_bdy(j,k,ii,P_XEB,1) = data3d(i,k,j)
       END DO
       END DO
       END DO
    ELSE IF ( char_stagger .EQ. 'V' ) THEN
    ...
    IF ( char_stagger .EQ. 'W' ) THEN
       ...
       DO i = MAX(ids,its) , MIN(ide-1,ite)
          space_bdy(i,k,j,P_YSB,1) = data3d(i,k,j)
       END DO
       END DO
       END DO
    ELSE IF ( char_stagger .EQ. 'M' ) THEN
    ...
    IF      ( char_stagger .EQ. 'V' ) THEN
       ...
       DO i = MAX(ids,its) , MIN(ide-1,ite)
          jj = jde - j + 1
          space_bdy(i,k,jj,P_YEB,1) = data3d(i,k,j)
       END DO
       END DO
       END DO
    ELSE IF ( char_stagger .EQ. 'U' ) THEN
...
!    REAL , DIMENSION(:,:,:,:) , INTENT(OUT) :: space_bdy
    ...
    IF ( char_stagger .EQ. 'W' ) THEN
       ...
       DO k = kds , kde
       DO i = MAX(ids,its) , MIN(ids + spec_bdy_width - 1,ite)
          space_bdy(j,k,i,P_XSB,1) = ( data3dnew(i,k,j) - data3dold(i,k,j) ) / time_diff
!         space_bdy(j,k,i,P_XSB,1) = 0. ! zeroout
       END DO
       END DO
       END DO
    ELSE IF ( char_stagger .EQ. 'M' ) THEN
...
!         space_bdy(j,k,i,P_XSB,1) = 0. ! zeroout
    ...
    IF      ( char_stagger .EQ. 'U' ) THEN
       ...
       DO i = MIN(ide,ite) , MAX(ide - spec_bdy_width + 1,its) , -1
          ii = ide - i + 1
          space_bdy(j,k,ii,P_XEB,1) = ( data3dnew(i,k,j) - data3dold(i,k,j) ) / time_diff
!         space_bdy(j,k,ii,P_XEB,1) = 0. ! zeroout
       END DO
       END DO
       END DO
    ELSE IF ( char_stagger .EQ. 'V' ) THEN
...
!         space_bdy(j,k,ii,P_XEB,1) = 0. ! zeroout
    ...
    IF ( char_stagger .EQ. 'W' ) THEN
       ...
       DO k = kds , kde
       DO i = MAX(ids,its) , MIN(ide-1,ite)
          space_bdy(i,k,j,P_YSB,1) = ( data3dnew(i,k,j) - data3dold(i,k,j) ) / time_diff
!         space_bdy(i,k,j,P_YSB,1) = 0. ! zeroout
       END DO
       END DO
       END DO
    ELSE IF ( char_stagger .EQ. 'M' ) THEN
...
!         space_bdy(i,k,j,P_YSB,1) = 0. ! zeroout
    ...
    IF      ( char_stagger .EQ. 'V' ) THEN
       ...
       DO i = MAX(ids,its) , MIN(ide-1,ite)
          jj = jde - j + 1
          space_bdy(i,k,jj,P_YEB,1) = ( data3dnew(i,k,j) - data3dold(i,k,j) ) / time_diff
!         space_bdy(i,k,jj,P_YEB,1) = 0. ! zeroout
       END DO
       END DO
       END DO
    ELSE IF ( char_stagger .EQ. 'U' ) THEN
...
!         space_bdy(i,k,jj,P_YEB,1) = 0. ! zeroout
 ...
 SUBROUTINE stuff_bdy_ijk ( data3d , space_bdy , char_stagger , &
                             ijds , ijde , spec_bdy_width , &
                             ids, ide, jds, jde, kds, kde , &
                             ims, ime, jms, jme, kms, kme , & 
                             its, ite, jts, jte, kts, kte )
...
!    REAL , DIMENSION(:,:,:,:) , INTENT(OUT) :: space_bdy
    ...
    IF ( char_stagger .EQ. 'W' ) THEN
       ...
       DO j = MAX(jds,jts) , MIN(jde-1,jte)
       DO i = MAX(ids,its) , MIN(ids + spec_bdy_width - 1,ite)
          space_bdy(j,k,i,P_XSB,1) = data3d(i,j,k)
       END DO
       END DO
       END DO
    ELSE IF ( char_stagger .EQ. 'M' ) THEN
    ...
    IF      ( char_stagger .EQ. 'U' ) THEN
       ...
       DO i = MIN(ide,ite) , MAX(ide - spec_bdy_width + 1,its) , -1
          ii = ide - i + 1
          space_bdy(j,k,ii,P_XEB,1) = data3d(i,j,k)
       END DO
       END DO
       END DO
    ELSE IF ( char_stagger .EQ. 'V' ) THEN
    ...
    IF ( char_stagger .EQ. 'W' ) THEN
       ...
       DO j = MAX(jds,jts) , MIN(jds + spec_bdy_width - 1,jte)
       DO i = MAX(ids,its) , MIN(ide-1,ite)
          space_bdy(i,k,j,P_YSB,1) = data3d(i,j,k)
       END DO
       END DO
       END DO
    ELSE IF ( char_stagger .EQ. 'M' ) THEN
    ...
    IF      ( char_stagger .EQ. 'V' ) THEN
       ...
       DO i = MAX(ids,its) , MIN(ide-1,ite)
          jj = jde - j + 1
          space_bdy(i,k,jj,P_YEB,1) = data3d(i,j,k)
       END DO
       END DO
       END DO
    ELSE IF ( char_stagger .EQ. 'U' ) THEN
...
!	endif
 ...
 SUBROUTINE stuff_bdytend_ijk ( data3dnew , data3dold , time_diff , space_bdy , char_stagger , &
                             ijds , ijde , spec_bdy_width , &
                             ids, ide, jds, jde, kds, kde , &
                             ims, ime, jms, jme, kms, kme , & 
                             its, ite, jts, jte, kts, kte )
...
!    REAL , DIMENSION(:,:,:,:) , INTENT(OUT) :: space_bdy
    ...
    IF ( char_stagger .EQ. 'W' ) THEN
       ...
       DO j = MAX(jds,jts) , MIN(jde-1,jte)
       DO i = MAX(ids,its) , MIN(ids + spec_bdy_width - 1,ite)
          space_bdy(j,k,i,P_XSB,1) = ( data3dnew(i,j,k) - data3dold(i,j,k) ) / time_diff
!         space_bdy(j,k,i,P_XSB,1) = 0. ! zeroout
       END DO
       END DO
       END DO
...
!         space_bdy(j,k,i,P_XSB,1) = 0. ! zeroout
    ...
    IF      ( char_stagger .EQ. 'U' ) THEN
       ...
       DO i = MIN(ide,ite) , MAX(ide - spec_bdy_width + 1,its) , -1
          ii = ide - i + 1
          space_bdy(j,k,ii,P_XEB,1) = ( data3dnew(i,j,k) - data3dold(i,j,k) ) / time_diff
!         space_bdy(j,k,ii,P_XEB,1) = 0. ! zeroout
       END DO
       END DO
       END DO
...
!         space_bdy(j,k,ii,P_XEB,1) = 0. ! zeroout
    ...
    IF ( char_stagger .EQ. 'W' ) THEN
       ...
       DO j = MAX(jds,jts) , MIN(jds + spec_bdy_width - 1,jte)
       DO i = MAX(ids,its) , MIN(ide-1,ite)
          space_bdy(i,k,j,P_YSB,1) = ( data3dnew(i,j,k) - data3dold(i,j,k) ) / time_diff
!         space_bdy(i,k,j,P_YSB,1) = 0. ! zeroout
       END DO
       END DO
       END DO
...
!         space_bdy(i,k,j,P_YSB,1) = 0. ! zeroout
    ...
    IF      ( char_stagger .EQ. 'V' ) THEN
       ...
       DO i = MAX(ids,its) , MIN(ide-1,ite)
          jj = jde - j + 1
          space_bdy(i,k,jj,P_YEB,1) = ( data3dnew(i,j,k) - data3dold(i,j,k) ) / time_diff
!         space_bdy(i,k,jj,P_YEB,1) = 0. ! zeroout
       END DO
       END DO
       END DO
