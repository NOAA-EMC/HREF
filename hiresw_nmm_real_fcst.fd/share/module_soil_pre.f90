         ...
         DO i = its , MIN(ide-1,ite)
            ...
            IF ( dominant_value .EQ. lwthresh ) THEN
               DO l = 1 , num_veg_cat
                ...
                write(message, FMT='(I4,I4,A,I4,A,I4)') i,j,' water and category ',l,' both at 50%, landmask is ',landmask(i,j)
...
CALL wrf_debug(0,message)
   ...
   SUBROUTINE process_soil_real ( tsk , tmn , &
      ...
      ELSE IF ( ( sf_surface_physics .EQ. 2 .or. sf_surface_physics .EQ. 99 ) .AND. ( num_soil_layers .GT. 1 ) ) THEN
...
cycle
!                 CALL wrf_error_fatal3 ( "module_soil_pre.b" , 2040 ,  TOPOSOIL values have large negative values < -1000 m, unrealistic. )
               ENDIF
...
cycle
                  CALL wrf_error_fatal3 ( "module_soil_pre.b" , 2048 ,  'TOPOSOIL values have large positive values > 10,000 m , unrealistic.' )
               ENDIF
...
cycle
                  CALL wrf_error_fatal3 ( "module_soil_pre.b" , 2058 ,  'TOPOSOIL difference with terrain elevation differs by more than 3000 m, unrealistic' )
               ENDIF
   ...
   SUBROUTINE init_soil_depth_1 ( zs , dzs , num_soil_layers )
      ...
      IF ( num_soil_layers .NE. 5 ) THEN
         ...
         CALL wrf_error_fatal3 ( "module_soil_pre.b" , 2156 ,  message )
      END IF
   ...
   SUBROUTINE init_soil_depth_2 ( zs , dzs , num_soil_layers )
      ...
      IF ( num_soil_layers .NE. 4 ) THEN
         ...
         CALL wrf_error_fatal3 ( "module_soil_pre.b" , 2185 ,  message )
      END IF
...
!      dzs = (/ 0.00 , 0.125, 0.175 , 0.70 , 1.30 , 1.40 /)
     ENDIF
      ...
      IF ( num_soil_layers .EQ. 4 .OR. num_soil_layers .EQ. 5 ) THEN
         ...
         CALL wrf_error_fatal3 ( "module_soil_pre.b" , 2224 ,  message )
      END IF
...
!new
!       write(message,*) st_input(1) in init_soil_2_real
!       CALL wrf_message(message)
	DO J=MIN(jde-1,jte),JTS, -MIN(jde-1,jte)/15
!       write(message,616) (st_input(I,1,J),I=its , MIN(ide-1,ite),MIN(ide-1,ite)/10)
!       CALL wrf_message(message)
	ENDDO

!       write(message,*) st_input(2) in init_soil_2_real
!       CALL wrf_message(message)
	DO J=MIN(jde-1,jte),JTS, -MIN(jde-1,jte)/15
!       write(message,616) (st_input(I,2,J),I=its , MIN(ide-1,ite),MIN(ide-1,ite)/10)
!       CALL wrf_message(message)
	ENDDO
  ...
  616	format(20(f4.0,1x))
      ...
      ELSE
         CALL wrf_error_fatal3 ( "module_soil_pre.b" , 2492 ,  &
   ...
   SUBROUTINE init_soil_3_real ( tsk , tmn , smois , tslb , &
      ...
      IF ( ( num_st_levels_input .LE. 0 ) .OR. & 
         ...
         CALL wrf_error_fatal3 ( "module_soil_pre.b" , 2737 ,  'no soil data' )
      ELSE
         IF ( flag_soilt000 .eq. 1 ) THEN
