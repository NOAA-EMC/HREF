...
cat namelist.input | sed s:nsoil:nsoilnamelist:g > namelist.input.mod
mv namelist.input.mod namelist.input
...
then
! into tomorrow
end=`$bindir/tomorrow $endtime`
YYYY=`echo $end | cut -c1-4`
MM=`echo $end | cut -c5-6`
DD=`echo $end | cut -c7-8`
fi

typeset -Z2 hrend

echo "hrend now: " $hrend

FINALDATE=${YYYY}-${MM}-${DD}T${hrend}:00:00
...
cp /emc2/wx20py/wrf_nmm/WRFV1/test/nmm_real/wrf_cntrl_central04.all ./wrf_cntrl.parm
