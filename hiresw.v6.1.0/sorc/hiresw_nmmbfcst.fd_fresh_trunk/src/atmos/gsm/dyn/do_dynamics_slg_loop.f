...
!!
      ...
      real(kind=kind_evod), allocatable, save :: save_qo_ls(:,:)
...
!
     ...
     &        ltem, ktem, lons_lat, mtem, jt, kt
     &,       kp_t,kp_y,kp_d,kp_z,kp_x,kp_w, stp, nt
...
!         ldfi_spect = .false.
          ...
          kdtdfi = kdt + ndfih
!
          if ( me == 0 ) print *,' calling spectdfini with ndfih=',ndfih
          if (ndfih /= 0 ) then
...
!
!     write(150+me,*)' bef grid_to_spec anl2 maxan2=',
!    &maxval(anl_gr_a_2(:,385,:)),' mingr=',minval(anl_gr_a_2(:,385,:))
...
!               ktem = p_w  + k - 1
!               ltem = p_ze + k - 1
!     write(150+me,*)' trie=',trie_ls(1,1,ktem),trie_ls(1,1,jtem)
!    &,' bfilte=',bfilte(1)
!     write(150+me,*)' trio=',trio_ls(1,1,ktem),trio_ls(1,1,jtem)
...
!$omp parallel do private(k,i,jt,kt)
     ...
     &                               (trie_ls(i,2,jt)-trie_ls(i,2,kt))
                 enddo
!     if (nt == ntrac) then
!     write(150+me,*)' filter trie_clw=',trie_ls(1,1,p_rt+levh-1)
!    &,'p_rt+levh-1=',p_rt+levh-1,trie_ls(1,1,p_rq+levh-1)
!    &,'p_rq+levh-1=',p_rq+levh-1,'bfilte=',bfilte(1)
!     endif
                 do i=1,len_trio_ls
...
!     write(0,*)' p_zq00=',trie_ls(1,1,p_zq)
!!
...
!     write(150+me,*)' trie_ls_kp_x=', trie_ls(1,1,p_rq+levh-1)
...
!      if(kdt<=1) then
!        write(0,*)'in do_dynamics_slg_loop kdt=',kdt,
!    &' g_q=',g_q,' g)u=',g_u,' g_v=',g_v,' g_t=',g_t,' g_rt=',g_rt
!        write(0,*)'in do_dynamics_slg_loop kdt=',kdt,
!    &   'ps=',maxval(grid_gr(:,g_q)),  minval(grid_gr(:,g_q)),
!    &   'u=',maxval(grid_gr(:,g_u)),   minval(grid_gr(:,g_u)),
!    &   'v=',maxval(grid_gr(:,g_v)),   minval(grid_gr(:,g_v)),
!    &   't=',maxval(grid_gr(:,g_t)),   minval(grid_gr(:,g_t)),
!    &   'rq=',maxval(grid_gr(:,g_rt)), minval(grid_gr(:,g_rt)),
!    &  'psg=',maxval(grid_gr(:,g_zq)), minval(grid_gr(:,g_zq))
!      endif
!
        stp = 1
