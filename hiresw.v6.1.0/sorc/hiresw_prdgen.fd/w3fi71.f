...
C   03-02-03  ROGERS       ADDED 1/8 LAT/LON LDAS GRID #110
C
...
C  B-grids
C  95=Puerto Rico nest
C  96=Hawaii nest
C  97=CONUS nest
C  98=1 km DC nest
C  99=Parent
      DATA  GRD95 / 0, 255,205,241,241,  17500, -71050, 136,    54000,
     & -106000 ,  032,027,64, 18926, -61192, 0, 0, 0/
      DATA  GRD96 / 0, 255,205,373,561,  11625, -156339, 136,   54000,
     & -106000,   032,027,64, 30429, -157827, 0, 0, 0/
      DATA  GRD97 / 0, 255,205,1371,1100, 15947,-125468, 136,   54000,
     & -106000,    042,036,64,45407,-52390, 0, 0, 0/
      DATA  GRD98 / 0, 255,205,343,343, 37835, -79392, 136,   39000,
     & -98000,    009,009,64, 40120, -74535, 0, 0, 0/
!
!-- Format for GRD99 grid is:
!   IM, JM - # of I, J grid points
!   SW_LAT, SW_LON - latitude, longitude of SW corner of grid
!   CEN_LAT, CEN_LON - central latitude, longtiude of grid
!   DLON, DLON - delta longitude and latitude of grid
!   DATA  GRD99 / 0, 255,205,IM,JM,  1000*SW_LAT, 1000*SW_LON, 136,   1000*CEN_LAT,
!  & 1000*CEN_LON,    1000*DLON, 1000*DLAT, 64, 0, 0, 0, 0, 0/
!
!   NOTE:  To get the SW corner mass point, search the "geo.log" NPS output file
!          for the string "return xlat, xlon(I,J):  1.000000000 1.000000000".  The
!          first block of numbers are for mass points, the second set are for velocity
!          points (the latter are always to the NE of the former).  Be sure to select
!          the correct task, since prints are generated for each task.
!
      DATA  GRD99 / 0, 255,205,1300,875,  20031, -122659, 136,   39000,
     & -98000,    037, 036, 64, 49814, -60593, 0, 0, 0/
     ...
     &    -938,    95, 938, 0, 0, 0, 0, 0, 0/
      DATA  GRD151/ 0, 255, 5, 478,429,  -7450, -144140,   8, -110000,
     &   33812,  33812, 0, 64, 0, 0, 0, 0, 0/
      DATA  GRD170/ 0, 255, 4, 512,256,  89463,       0, 128,  -89463,
     &    -703,   128, 703, 0, 0, 0, 0, 0, 0/
      DATA  GRD179/ 0, 255, 5,1196,817, -2500,   217500,   8,  -100000,
     ...
     & -130124,    108,  108,  0, 0, 0, 0, 0, 0/
      ...
      DATA  GRD189/ 0, 255, 3, 1295,854,  20800, -122000,   8,  -98000,
     &   4000,  4000, 0, 64, 0, 39000, 39000, 0, 0/

      DATA  GRD190/ 0, 255, 3, 318,316,  37900, -79100,   8,  -98000,
     &   1000,  1000, 0, 64, 0, 39000, 39000, 0, 0/
...
!     & -111000,    577,538,64, 0, 0, 0, 0, 0/
     ...
     & -157350,     53, 53,64, 0, 0, 0, 0, 0/
      DATA  GRD195/ 0, 255, 1, 177,129,  16829,  -68195, 128,   19736,
     &  -63984, 2500, 2500, 20000, 64, 0, 0, 0, 0/
