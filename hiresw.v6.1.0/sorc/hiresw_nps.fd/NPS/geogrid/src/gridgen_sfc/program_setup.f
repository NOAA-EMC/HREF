 ...
 use consts, only       : lonsperlat_t878,   &
                          lonsperlat_t574,   &
                          lonsperlat_t382,   &
                          lonsperlat_t254,   &
                          lonsperlat_t190,   &
...
! logical, parameter    :: grib2=.false.   ! output in grib2 or grib1
 logical               :: grib2          ! output in grib2 or grib1
   ...
   case default
     ...
     select case (trim(domain_type))
       ...
       case("gaussian")
         ...
         dy_mdl    = resol_mdl   
         if (run_global_thinned) then
           print*,"- TO RUN THINNED, NEED TO ADD LONSPERLAT INFO FOR THIS GRID."
