...
real ,DIMENSION(grid%sm31:grid%em31,grid%sm32:grid%em32,grid%spec_bdy_width,num_tracer) :: tracer_btye
   ...
   real ,DIMENSION(grid%sm31:grid%em31,grid%sm32:grid%em32,grid%sm33:grid%em33) :: h_tendency, &
                                                                                   z_tendency
   LOGICAL :: tenddec
   LOGICAL :: diag_flag, diag_flag_alt
...
real ,DIMENSION(grid%sm31:grid%em31,grid%sm33:grid%em33) :: w_colmean
real ,DIMENSION(grid%sm31:grid%em31,grid%sm33:grid%em33) :: numcolpts
real ,DIMENSION(grid%sm31:grid%em31,grid%sm33:grid%em33) :: grpl_colint
...
real ,DIMENSION(grid%sm31:grid%em31,grid%sm32:grid%em32,grid%sm33:grid%em33,num_tracer) :: tracer_old
   ...
   if ( Is_alarm_tstep(grid%domain_clock, grid%alarms(HISTORY_ALARM)) ) then
      diag_flag = .true.
   elseif (mod(grid%itimestep,5) .eq. 0) then
      diag_flag_alt = .true.
   else
      diag_flag = .false.
      diag_flag_alt = .false.
...
CALL PERIOD_BDY_EM_E_sub ( grid, &
  ...
  ips, ipe, jps, jpe, kps, kpe )
   ...
   CALL diagnostic_output_calc( &
      ...
      & ,Q2=grid%q2,PSFC=grid%psfc &
      & ,T2MAX=grid%t02_max,T2MIN=grid%t02_min &
      & ,RH2MAX=grid%rh02_max,RH2MIN=grid%rh02_min &
