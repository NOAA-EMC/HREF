...
!***    Need an ESMF_ClockGet call here to set the alarm rings
!***    for the non-filter integration in restarted runs.  Without it there are problems
!***    with non-zero hour restarts not writing the requested output beyond the initial time
!
            CALL ESMF_ClockGet(clock   =CLOCK_INTEGRATE                         &
