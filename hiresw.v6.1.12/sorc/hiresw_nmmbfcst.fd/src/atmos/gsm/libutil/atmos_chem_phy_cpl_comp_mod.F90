...
!---------------------------------------------
      ...
      write(0,*)' after getpointer maxdu=',maxval(p_du001),' mindu=',minval(p_du001)
       endif
...
! ---  data copy between phy export state and chem export state
         ...
         if ( run_DU ) then
           do j=1,jm
             ...
             do i=1,lons_lat
               p_du001(i,j,1:km) = p_du001(i,j,km:1:-1)
               p_du002(i,j,1:km) = p_du002(i,j,km:1:-1)
               p_du003(i,j,1:km) = p_du003(i,j,km:1:-1)
               p_du004(i,j,1:km) = p_du004(i,j,km:1:-1)
               p_du005(i,j,1:km) = p_du005(i,j,km:1:-1)
             enddo
           enddo
         endif
         ...
         if ( run_SS ) then
           do j=1,jm
             lons_lat = lonsperlar_r(j)
             do i=1,lons_lat
               p_ss001(i,j,1:km) = p_ss001(i,j,km:1:-1)
               p_ss002(i,j,1:km) = p_ss002(i,j,km:1:-1)
               p_ss003(i,j,1:km) = p_ss003(i,j,km:1:-1)
               p_ss004(i,j,1:km) = p_ss004(i,j,km:1:-1)
               p_ss005(i,j,1:km) = p_ss005(i,j,km:1:-1)
             enddo
           enddo
         endif
        ...
        if ( run_BC ) then
          do j=1,jm
            lons_lat = lonsperlar_r(j)
            do i=1,lons_lat
              p_bcphilic(i,j,1:km) = p_bcphilic(i,j,km:1:-1)
              p_bcphobic(i,j,1:km) = p_bcphobic(i,j,km:1:-1)
            enddo
          enddo
        endif
        ...
        if ( run_OC ) then
          do j=1,jm
            lons_lat = lonsperlar_r(j)
            do i=1,lons_lat
              p_ocphilic(i,j,1:km) = p_ocphilic(i,j,km:1:-1)
              p_ocphobic(i,j,1:km) = p_ocphobic(i,j,km:1:-1)
            enddo
          enddo
        endif
        ...
        if ( run_SU ) then
          do j=1,jm
            lons_lat = lonsperlar_r(j)
            do i=1,lons_lat
              p_msa(i,j,1:km) = p_msa(i,j,km:1:-1)
              p_so2(i,j,1:km) = p_so2(i,j,km:1:-1)
              p_so4(i,j,1:km) = p_so4(i,j,km:1:-1)
              p_dms(i,j,1:km) = p_dms(i,j,km:1:-1)
            enddo
          enddo
        endif
