...
!-----------------------------------------------------------------------
 ...
 if (iret /= 0) goto 9000
        if (abs(dummy(1)) .lt. 1.) then
        print*, 'converting NAM latitude from radians to degrees'
        dummy=dummy*r2d
        endif
 lat11=dummy(1)
 ...
 call nemsio_readrecv(gfile,vname,vlevtyp,vlev,dummy,iret=iret)
 if (iret /= 0) goto 9000
        if (abs(dummy(1)) .lt. 3.) then
        print*, 'converting NAM longitude from radians to degrees'
        dummy=dummy*r2d
        endif
 lon11=dummy(1)
