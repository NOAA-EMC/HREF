...
!-------------------------------------------------------------------
	...
	SUBROUTINE extra_nems_fields( gridin, gridout )
...
!       North pole ice
	if (gridout%GLOBAL .and. JTE .eq. JDE-1) then
	...
	J=JDE-1
      ...
      DO J=JTE,JTS,min(-JTE/25,-1)
         write(0,637) (gridout%SICE(I,J),I=ITS,ITE,max(1,ITE/20))
      END DO
        endif
...
!!!!!!!!!!!!!!!!!!!!!!!!!
      ...
      DO J = jts, MIN(jde-1,jte)
        DO I = its, MIN(ide-1,ite)
           IF (gridout%TG(I,J) .lt. 200.) THEN   ! only use TG_ALT definition if
