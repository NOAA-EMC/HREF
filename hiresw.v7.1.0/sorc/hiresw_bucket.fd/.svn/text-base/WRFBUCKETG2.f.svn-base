...
C grib2
      ...
      INTEGER,DIMENSION(:) :: PDS_SNOWD_HOLD(200)
      INTEGER,DIMENSION(:) :: PDS_SNOW_HOLD_EARLY(200),
     &                        PDS_SNOWD_HOLD_EARLY(200),
     &                        DRS_SNOWD_HOLD_EARLY(200),
...
C grib2
	...
	real:: sd_later(IM*JM),sd_earlier(IM*JM),sdprecip(im*jm)
...
! ================================================
     ...
     &     UNPACK,K,GFLD,IRET)
        ...
        write(0,*) 'snowd pulled gfld%ipdtnum: ', gfld%ipdtnum
C        do J=1,15
C        write(0,*) 'snowd J,gfld%ipdtmpl(J): ', J,gfld%ipdtmpl(J)
C        enddo

	if (IRET .ne. 0) then
	write(0,*) 'bad getgb snowd earlier ', IRET
	STOP
	endif
        ...
        do K=1,gfld%ipdtlen
        write(0,*) 'snowd K,gfld%ipdtmpl(K): ', K,gfld%ipdtmpl(K)
        PDS_SNOWD_HOLD_EARLY(K)=gfld%ipdtmpl(K)
        enddo

        do K=1,gfld%idrtlen
	write(0,*) 'snowd K, gfld%idrtmpl(K): ', K, gfld%idrtmpl(K)
        DRS_SNOWD_HOLD_EARLY(K)=gfld%idrtmpl(K)
        enddo

        write(0,*) 'maxval(gfld%fld) for snowd earlier: ', 
     &                maxval(gfld%fld)
...
! ============================
	...
	if (IRET1 .ne. 0) then
	 write(0,*) 'bad getgb snowd later ', IRET1
	STOP
	endif

        write(0,*) 'set sd_later to gfld%fld'
        sd_later=gfld%fld

        do K=1,gfld%ipdtlen
        PDS_SNOWD_HOLD(K)=gfld%ipdtmpl(K)
        enddo

        write(0,*) 'maxval(sd_later): ', maxval(sd_later)
...
! ============================
	...
	sdprecip(NPT)=sd_later(NPT)-sd_earlier(NPT)
	enddo
...
!! pump up precision?  Difference values seem wrong
        do K=1,gfld%idrtlen
        gfld%idrtmpl(K)=DRS_SNOWD_HOLD_EARLY(K)
        enddo
        ...
        gfld%fld=sdprecip
	call putgb2(13,GFLD,IRET)
        write(0,*) 'IRET from putgb2 for sdprecip ', IRET


        call baclose(13,IRET)

	write(0,*) 'extremes of precip,snow: ', 
     +		maxval(dprecip),maxval(sprecip),maxval(sdprecip)
