...
!jw
      ...
      USE namelist_physics_def,                                                 &
                          ONLY: ras, jo3, ldiag3d, ngptc, ens_nam,              &
                                reduced_grid, grid_aldata, nst_fcst,            &
                                isol, ico2, ialb, iems, iaer, iovr_sw,          &
                                iovr_lw,ictm, isubc_sw,isubc_lw,                &
                                sashal, crick_proof, ccnorm, norad_precip,      &
...
!*    USE tracer_const,   ONLY: set_tracer_const
      ...
      USE gfs_physics_sfc_flx_set_mod,                                  &
                          ONLY: sfcvar_aldata, flxvar_aldata, flx_init
...
!
      ...
      write(0,*)' dxmax=',dxmax,' dxmin=',dxmin,' dxinv=',dxinv
...
!     ALLOCATE (gis_phy%CLDCOV(LEVS,LONR,LATS_NODE_R_MAX), stat = ierr)
      ...
      ALLOCATE (gis_phy%sstForGSM(LONR,LATS_NODE_R),    stat = ierr)
      gis_phy%sstForGSM = -99999.0
...
!
      allocate (gis_phy%phy_f3d(NGPTC,LEVS,num_p3d+npdf3d,NBLCK,lats_node_r), stat = ierr)
!     allocate (gis_phy%phy_f3d(NGPTC,LEVS,NBLCK,lats_node_r,num_p3d+npdf3d), stat = ierr)
...
!     if (icolor /= 2 .or. .not. liope) then
        if (num_p3d > 0) gis_phy%phy_f3d = 0.0
        if (num_p2d > 0) gis_phy%phy_f2d = 0.0
!     endif
!jw
