...
#include "wrf_status_codes.h"
      ...
      if (my_proc_id == IO_NODE .or. do_tiled_input) then
         ...
         if (io_form_input == NETCDF_FORMAT) suffix='nc'
         if (io_form_input ==    DIO_FORMAT) suffix='dio'

         input_fname = ' '
         if (gridtype == 'C') then
            input_fname = trim(opt_output_from_geogrid_path)//'geo_em.d  .'//trim(suffix)
            i = len_trim(opt_output_from_geogrid_path)
            write(input_fname(i+9:i+10),'(i2.2)') nest_number
         else if (gridtype == 'E') then
            input_fname = trim(opt_output_from_geogrid_path)//'geo_nmm.d  .'//trim(suffix)
            i = len_trim(opt_output_from_geogrid_path)
            write(input_fname(i+10:i+11),'(i2.2)') nest_number
         else if (gridtype == 'B') then
            input_fname = trim(opt_output_from_geogrid_path)//'geo_nmb.d  .'//trim(suffix)
            i = len_trim(opt_output_from_geogrid_path)
            write(input_fname(i+10:i+11),'(i2.2)') nest_number
         else if (gridtype == 'A') then
            input_fname = trim(opt_output_from_geogrid_path)//'geo_sla.d  .'//trim(suffix)
            i = len_trim(opt_output_from_geogrid_path)
            write(input_fname(i+10:i+11),'(i2.2)') nest_number
         end if
...
#include "wrf_status_codes.h"
      ...
      if (my_proc_id == IO_NODE .or. do_tiled_input) then
         ...
         if (num_calls == 0) then
#ifdef IO_NETCDF
            if (io_form_input == NETCDF_FORMAT) call ext_ncd_get_next_time(handle, datestr, istatus)
#endif
         end if
...
#ifdef IO_NETCDF
         if (io_form_input == NETCDF_FORMAT) call ext_ncd_get_next_var(handle, cname, istatus)
#endif
      end if
...
#endif
#ifdef IO_DIO
      if (io_form_input == DIO_FORMAT) call dio_read(dfile,trim(var_name), var_value(1:n), iret=istatus)
#endif
      call mprintf((istatus /= 0),ERROR,'Error while reading domain time-independent attribute.')
