      ...
      do while (ifieldstatus == 0)  !{
         ...
         if (ifieldstatus == 0) then !{
            ...
            if (istagger == M) then
               ...
               write(0,*)' Unknown stagger ', fieldname, istagger
            end if
...
!      for staggering if it is alright to pass true domain dimensions to write_field.
               ...
               LMAX=size(real_dom_array,dim=3)
              if (LMAX .gt. 1) then

                do LL=1,LMAX
                 write(lchar,47) LL
  47             format(I2.2)
                 recname=trim(fields(i)%fieldname)//'_'//lchar
                 call dio_write(dfile,trim(recname), &
                              real_dom_array(sd(1):ed(1),sd(2):ed(2),LL:LL), &
...
!          normal write for single layer data
