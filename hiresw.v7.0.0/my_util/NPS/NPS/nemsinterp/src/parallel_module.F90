...
!	write(0,*) 'MPI_INIT called', mpi_ierr
...
! BUG: Check on mpi_ierr
   ...
   subroutine gather_whole_field_r(patch_array, ms1, me1, ms2, me2, ms3, me3, &
	...
	write(0,*) 'inside gather_whole_field_r : ', my_proc_id
      if (my_proc_id == IO_NODE) then
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
   ...
   !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
   ! Name: make_global_bdy
   !
   ! Purpose: Assemble local bdy information onto global arrays
   !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!

      SUBROUTINE make_global_bdy(ARRAY,GLOB_ARRAY,MYPE,NPES,MPI_COMM_COMP       &
     &              ,IDS,IDE,JDS,JDE,KDS,KDE                           &
     &              ,IMS,IME,JMS,JME,KMS,KME, do_work )
!----------------------------------------------------------------------
!**********************************************************************
!
      IMPLICIT NONE
...
!
      REAL,DIMENSION(IMS:IME,JMS:JME,KMS:KME,2),INTENT(IN) :: ARRAY
      REAL,DIMENSION(IDS:IDE,JDS:JDE,KDS:KDE,2),INTENT(OUT) :: GLOB_ARRAY
      LOGICAL,INTENT(IN) :: do_work(0:NPES-1)
!
!
!*** LOCAL VARIABLES
!
...
!
      INTEGER :: IENDX,IER,IPE,IRECV,IRTN,ISEND,IUNIT                  &
     &          ,N,NLEN,NSIZE
...
!----------------------------------------------------------------------
      ...
      IF(MYPE==0)THEN
        ...
        DO J=JMS,JME
        DO I=IMS,IME
          GLOB_ARRAY(I,J,K,M)=ARRAY(I,J,K,M)
        ENDDO
        ENDDO
        ENDDO
        ENDDO
...
!
          ...
          JTE_REM=JT_REM(2)
!
          NSIZE=(ITE_REM-ITS_REM+1)*(JTE_REM-JTS_REM+1)*(KDE-KDS+1)*2
...
!----------------------------------------------------------------------
      ELSE
      IF (do_work(MYPE)) THEN
        NSIZE=(IME-IMS+1)*(JME-JMS+1)*(KDE-KDS+1)*2
        ALLOCATE(VALUES(1:NSIZE))
!
        N=0
        DO M=1,2
...
!----------------------------------------------------------------------
!
      END SUBROUTINE make_global_bdy

   !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
   ! Name: make_global_bdy_2d
   !
   ! Purpose: Assemble local bdy information onto global arrays
   !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!

      SUBROUTINE make_global_bdy_2d(ARRAY,GLOB_ARRAY,MYPE,NPES,MPI_COMM_COMP       &
     &              ,IDS,IDE,JDS,JDE,KDS,KDE                           &
     &              ,IMS,IME,JMS,JME,KMS,KME, do_work )
!----------------------------------------------------------------------
!**********************************************************************
...
!***
!***
!*** NOTE:  HERE THE IMS/IME,JMS/JME LIMITS ARE SIMPLY THE LOCAL
!***        PATCH DIMENSIONS
!***
!***        IDS/IDE, JDS/JDE ARE THE MORE TYPICAL DOMAIN LIMITS
!
!
      INTEGER,INTENT(IN) :: IDS,IDE,JDS,JDE,KDS,KDE                    &
     &                     ,IMS,IME,JMS,JME,KMS,KME                    &
     &                     ,MPI_COMM_COMP,MYPE,NPES
!
      REAL,DIMENSION(IMS:IME,JMS:JME,2),INTENT(IN) :: ARRAY
      REAL,DIMENSION(IDS:IDE,JDS:JDE,2),INTENT(OUT) :: GLOB_ARRAY
      LOGICAL,INTENT(IN) :: do_work(0:NPES-1)
!
!
!*** LOCAL VARIABLES
...
!----------------------------------------------------------------------
!
      END SUBROUTINE make_global_bdy_2d

!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!

      SUBROUTINE make_global(ARRAY,GLOB_ARRAY,MYPE,NPES,MPI_COMM_COMP       &
     &              ,IDS,IDE,JDS,JDE,KDS,KDE                           &
     &              ,IMS,IME,JMS,JME,KMS,KME                           &
...
!----------------------------------------------------------------------
     ...
     &                     ,MPI_COMM_COMP,MYPE,NPES
!
      REAL,DIMENSION(ITS:ITE,JTS:JTE,KDS:KDE),INTENT(IN) :: ARRAY
      REAL,DIMENSION(IDS:IDE,JDS:JDE,KDS:KDE),INTENT(OUT) :: GLOB_ARRAY
!
!
!*** LOCAL VARIABLES
!
...
!
          ...
          JTE_REM=JT_REM(2)
!
          NSIZE=(ITE_REM-ITS_REM+1)*(JTE_REM-JTS_REM+1)*(KDE-KDS+1)
          ALLOCATE(VALUES(1:NSIZE))
!
          CALL MPI_RECV(VALUES,NSIZE,MPI_REAL,IPE,IPE                   &
     &                 ,MPI_COMM_COMP,JSTAT,IRECV)
...
!----------------------------------------------------------------------
      ELSE
        NSIZE=(ITE-ITS+1)*(JTE-JTS+1)*(KDE-KDS+1)
        ALLOCATE(VALUES(1:NSIZE))
!
        N=0
        DO K=KDS,KDE
