...
!                 i_parent_start_loc, j_parent_start_loc, &
      ...
      character (len=2) :: nestnum  !Modified by Thiago: changed "len=1" to "len=2".
...
!   have two lists of contents retrieved...full for initial time, limited for bdy times?
	...
	filename='                                                           '
        ...
        if (gridtype == 'B') then
          filename(1:9)='met_nmb.d'     !Modified by Thiago: Changed indices from "1:10" to "1:9" to exclude last character '0'.
        elseif (gridtype == 'A') then
          filename(1:9)='met_sla.d'     !Modified by Thiago: Changed indices from "1:10" to "1:9" to exclude last character '0'.
	endif
...
!
	...
	write(0,*) 'define nestnum'
 	write(nestnum,633) n
  633   format(I0.2)    !Modified by Thiago: Changed format "I1" to "I0.2" to include a leading zero for vales of "n<10".

        filename(10:11)=nestnum(1:2)    !Modified by Thiago: Changed indices "11:11" to "10:11" to include the first two characters.
        filename(12:12)='.'
	filename(13:31)=valid_date

#ifdef IO_NETCDF
      if (io_form_input == NETCDF_FORMAT) filename(32:34)='.nc'
#endif
#ifdef IO_DIO
      if (io_form_input == DIO_FORMAT) filename(32:35)='.dio'
#endif
	...
	write(0,*) 'sn_patch_s sn_patch_e : ', sn_patch_s, sn_patch_e
        ...
        if (we_mem_s .eq. 1 .and. sn_mem_s .eq. 1) then
         print_diag=.true.
        else
         print_diag=.false.
        endif
...
!!!! use memory dimensions to allow for easier data exchanges later?
	...
	write(0,*) 'no_seaice into read_nps: ', no_seaice
	write(0,*) 'use_igbp into read_nps: ', use_igbp
        endif

        CALL read_nps (gridin,trim(filename),valid_date,num_metgrid_levels, &
                       ncep_processing, do_gwd,do_clouds,no_seaice,spectral,&
                       use_igbp,grib_src,we_patch_s, we_patch_e, sn_patch_s, sn_patch_e, &
	...
	if (print_diag) write(0,*) 'into vinterp_driver: ', num_metgrid_levels

        CALL vinterp_driver(gridin,gridout,w,num_metgrid_levels,gridbdy%lnsh,gridbdy%lnsv)
	...
	CALL extra_nems_fields(n, gridin, gridout)
        if (print_diag) then
	...
	write(0,*) 'gridout%cwm extremes, post extra_nems_fields: ', minval(gridout%CWM), maxval(gridout%CWM)
        ...
        if (print_diag) write(0,*) 'gridout%cwm extremes, post write_nemsinit: ', minval(gridout%CWM), maxval(gridout%CWM)

	ENDIF


        if (print_diag) write(0,*) 'call proc_bdy'

	if ( n .eq. 1) then
        CALL proc_bdy (gridout, gridin, gridbdy, t, n)
        if (print_diag) write(0,*) 'return proc_bdy'
	endif
	...
	if (global) exit TIMES
        ...
        if (print_diag) write(0,*) 'end of TIMES loop for time t: ', t
!	call summary()
      end do TIMES  ! Loop over n_times

        if (print_diag) write(0,*) 'call deallocate'
...
!      write(0,*) 'ERROR input_init(): Error opening input for domain : ', n
      ...
      write(0,*)io_form_input, trim(filename)
      ...
      call ext_get_dom_ti_integer_scalar('i_parent_start', i_parent_start_loc)
      call ext_get_dom_ti_integer_scalar('j_parent_start', j_parent_start_loc)
      ...
      write(0,*) 'Done reading static global attributes.'
...
! stub
	...
	type(output_vars) :: gridout
        ...
        if (ASSOCIATED(gridin_loc%CLWMR)) deallocate(gridin_loc%CLWMR)
        ...
        if (ASSOCIATED(gridin_loc%GWD_OROG)) deallocate(gridin_loc%GWD_OROG)
...
!	if (ASSOCIATED(gridout%GLON)) deallocate(gridout%GLON)
	...
	if (ASSOCIATED(gridout%SH2O)) deallocate(gridout%SH2O)
        if (ASSOCIATED(gridout%pint_out)) deallocate(gridout%pint_out)
...
!!!      work
	...
	if (ASSOCIATED(work%rimef_input)) deallocate(work%rimef_input)
        ...
        if (ASSOCIATED(gridout%ETA1)) deallocate(gridout%ETA1)
        if (ASSOCIATED(gridout%ETA2)) deallocate(gridout%ETA2)
        if (ASSOCIATED(gridout%AETA1)) deallocate(gridout%AETA1)
        if (ASSOCIATED(gridout%AETA2)) deallocate(gridout%AETA2)
        if (ASSOCIATED(gridout%DETA1)) deallocate(gridout%DETA1)
        if (ASSOCIATED(gridout%DETA2)) deallocate(gridout%DETA2)
        if (ASSOCIATED(gridout%NL_ETALEVS)) deallocate(gridout%NL_ETALEVS)


        if (ASSOCIATED(gridout%DZSOIL)) deallocate(gridout%DZSOIL)
        if (ASSOCIATED(gridout%SLDPTH)) deallocate(gridout%SLDPTH)
        if (ASSOCIATED(gridout%RTDPTH)) deallocate(gridout%RTDPTH)
