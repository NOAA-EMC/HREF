...
module gridinfo_module
   ...
   real :: known_lat, known_lon, pole_lat, pole_lon, stand_lon, truelat1, truelat2, &
           ...
           dlatdeg, dlondeg, domcenlat_loc, domcenlon_loc
   ...
   logical :: do_tiled_output, ncep_processing, do_gwd, just_last, use_igbp, &
              ncep_proc_grib2,movable_nests, ignore_gridgen_sfc
   integer :: debug_level
...
!      real :: dx, dy, rparent_gridpts
      ...
      namelist /geogrid/ parent_id, parent_grid_ratio, &
                         ...
                         map_proj, ref_x, ref_y, ref_lat, ref_lon, dom_cen_lat, dom_cen_lon, &
                         pole_lat, pole_lon, truelat1, truelat2, stand_lon, &
                         dx, dy, geog_data_res, geog_data_path, opt_geogrid_tbl_path, ncep_processing, &
                         ncep_proc_path, ncep_proc_prefix, ncep_proc_domain_type, do_gwd, just_last, &
                         ignore_gridgen_sfc, use_igbp, &
...
!      call mprintf(.true.,LOGFILE,'  DY                = %f',f1=dy)
      ...
      do i=1,max_dom
      call mprintf(.true.,LOGFILE,'  DOM_CEN_LAT       = %f',f1=dom_cen_lat(i))
      call mprintf(.true.,LOGFILE,'  DOM_CEN_LON       = %f',f1=dom_cen_lon(i))
      enddo
...
!      end if
      ...
      call mprintf(gridtype /= 'C' .and. gridtype /= 'E' .and. gridtype /= 'B' &
                   .and. gridtype /= 'A', ERROR, &
      ...
      else if ((index(map_proj, 'ROTATED_LLB') /= 0) .and. &
               (len_trim(map_proj) == len('ROTATED_LLB'))) then
        write(6,*) 'set iproj type to PROJ_ROTLLB: ', PROJ_ROTLLB
         iproj_type = PROJ_ROTLLB
      ...
      if (gridtype == 'E') then
         ...
         phi = dykm*real(jydim(1)-1)/2.
         lambda = dxkm*real(ixdim(1)-1)/2.
         print*, 'dykm, dxkm: ', dykm, dxkm
         do i=2,n_domains
!generalize	if ( mod((ixdim(i)-1),3) .ne. 0 .or.  mod((jydim(i)-1),3) .ne. 0 ) then
           if ( mod((ixdim(i)-1),parent_grid_ratio(i)) .ne. 0 .or.  mod((jydim(i)-1),parent_grid_ratio(i)) .ne. 0 ) then
               write(0,*) 'domain: ', I, ' doesnt coincide with parent at NE corner, but should be okay'
!         	write(0,*) 'reset such that ixdim_nest and jydim_nest minus one divide evenly into:', parent_grid_ratio(i)
!	        write(0,*) 'needed so the nest origin falls on the parent grid'
!                STOP
            endif
         end do
      end if
      ...
      else if (gridtype == 'C' .or. gridtype == 'A') then
         if (known_x == NAN .and. known_y == NAN) then
            known_x = ixdim(1) / 2.
            known_y = jydim(1) / 2.
         else if (known_x == NAN .or. known_y == NAN) then
            call mprintf(.true.,ERROR, &
                      'In namelist.nps, neither or both of ref_x, ref_y must be specified.')
         end if 
      end if
...
!         end do
        ...
        write(6,*) 'see gridtype as B'
         ! B grid supports only the rotated lat/lon projection
         if (iproj_type /= PROJ_ROTLLB) then
            call mprintf(.true., WARN, &
                         'For the NMB core, projection type must be rotated '// &
                         'lat/lon (map_proj=rotated_llb)')
            call mprintf(.true.,WARN,'Projection will be set to rotated_llb')
            iproj_type = PROJ_ROTLLB
         end if
         ...
         !   for the sake of being consistent with WRF namelist
!          call mprintf(mod(e_sn(1)+1,2) /= 0, ERROR, &
!        'For the NMB core, E_SN must be an even number for grid %i.', i1=1)

         do i=2,n_domains
            call mprintf((parent_grid_ratio(i) /= 3), WARN, &
                         'For the NMMB core, the parent_grid_ratio shouldnt need to be 3. '// &
                         'However, look carefully at output (new functionality).', i1=i) 
!            parent_grid_ratio(i) = 3
         end do
 ...
 1000 call mprintf(.true.,ERROR,'Error opening file namelist.nps')
