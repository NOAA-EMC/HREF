...
module process_tile_module
   ...
   subroutine process_tile(which_domain, grid_type, dynopt,        &
      ...
      integer :: landmask_value, min_category, max_category, min_level, max_level, &
                 smth_opt, smth_passes,funit,funit_two,funit_three
      ...
      integer :: istagger, dim1size,dim2size,dim3size
      logical,parameter :: REPLACE_TOPO=.false.

      real, allocatable:: alt_topo(:,:),alt_land(:,:),alt_stdh(:,:)
      ...
      f_array => NULL()
      e_array => NULL()
      mapfac_array_m_x => NULL()
      mapfac_array_u_x => NULL()
      mapfac_array_v_x => NULL()
      mapfac_array_m_y => NULL()
      mapfac_array_u_y => NULL()
      mapfac_array_v_y => NULL()
      sina_array => NULL()
      cosa_array => NULL()

	print*, 'top of process_tile'
	...
	print*, 'SW lat,lon :: ', corner_lats(1), corner_lons(1)
	print*, 'NE lat,lon :: ', corner_lats(3), corner_lons(3)
	print*, 'start_patch_i, end_patch_i: ', start_patch_i, end_patch_i
	print*, 'start_patch_j, end_patch_j: ', start_patch_j, end_patch_j
...
!!!
!!!  Maybe this location is a horrible one for reading in the
!!!  alternative topography data.  Other places more appropriate?
!!!

         if (REPLACE_TOPO) then

               dim1size=size(field,dim=1)
               dim2size=size(field,dim=2)
               dim3size=size(field,dim=3)

               allocate(alt_land(dim1size-6,dim2size-6))
               allocate(alt_topo(dim1size-6,dim2size-6))
               allocate(alt_stdh(dim1size-6,dim2size-6))

      do funit=10,100
         inquire(unit=funit, opened=is_used)
         if (.not. is_used) then
             open(unit=funit,file='mnts',form='unformatted')
             exit
         endif
      end do
               call mprintf(.true.,STDOUT,' HERE opened funit %i ' ,i1=funit)
               read(funit) alt_topo,alt_land,alt_stdh
               close(unit=funit)

! have SM, want LANDMASK for WPS/REAL processing

                do J=1,dim2size-6
                do I=1,dim1size-6
                  if (alt_land(I,J) .gt. 0.5) field(I,J,1)=0.
                  if (alt_land(I,J) .lt. 0.5) field(I,J,1)=1.
                enddo
                enddo
                ...
                deallocate(alt_topo)
                deallocate(alt_stdh)
               call mprintf(.true.,STDOUT,' writing ingested landmask ' )

         call write_field(start_mem_i, end_mem_i, start_mem_j, end_mem_j, 1, 1, 'LANDMASK', &
                          datestr, real_array=landmask)

         else
               call mprintf(.true.,STDOUT,' writing computed landmask ' )
         ...
         if (.not.only_save_dominant .or. (idomcatstatus /= 0)) then
            ...
            if (istatus == 0) then
               ...
               if (grid_type == 'C' .or. grid_type == 'B') then
            ...
            if (.not. hash_search(processed_fieldnames, temp_string)) then
               ...
               if (iget_fieldtype(fieldname,istatus) == CONTINUOUS) then
                  ...
                  if (istatus == 0) then
                     ...
                     if (grid_type == 'C' .or. grid_type == 'B' .or. grid_type == 'A') then
                        if (smth_opt == ONETWOONE) then
                           call one_two_one(field,                      &
                                            start_patch_i, end_patch_i, &
                                            start_patch_j, end_patch_j, &
                     ...
                     if (trim(fieldname) == 'STDH') then
                     call mprintf(.true.,STDOUT,'  dealing with STDH')
                     ...
                     if (REPLACE_TOPO) then
               call mprintf(.true.,STDOUT,' allocate alt_topo for topo ' )

                    allocate(alt_land(dim1size-6,dim2size-6))
                    allocate(alt_topo(dim1size-6,dim2size-6))
                    allocate(alt_stdh(dim1size-6,dim2size-6))
      do funit_two=10,100
         inquire(unit=funit_two, opened=is_used)
         if (.not. is_used) exit
      end do
               call mprintf(.true.,STDOUT,' HERE2 opening funit_two %i ' ,i1=funit_two)
                    open(unit=funit_two,file='mnts',form='unformatted')
                    read(funit_two) alt_topo,alt_land,alt_stdh
                    close(unit=funit_two)

                     do J=1,dim2size-6
                     do I=1,dim1size-6
                       field(I,J,1)=alt_stdh(I,J)
                     enddo
                     enddo
               ...
               call mprintf(.true.,STDOUT,' deallocate alt_topo for topo ' )
                     endif
...
!                                   min_level, max_level, trim(fieldname), datestr, real_array=field)
                     ...
                     call mprintf(.true.,STDOUT,'  dealing with HGT_M')
                     call mprintf(.true.,STDOUT,'  could insert Zavisa topography here')

                     dim1size=size(field,dim=1)
                     dim2size=size(field,dim=2)
                     dim3size=size(field,dim=3)

                     call mprintf(.true.,STDOUT,'  dimensions of field array are %i and %i and %i ', &
                                                i1=dim1size, i2=dim2size, i3=dim3size)
                     call mprintf(.true.,STDOUT,'  pull a topography into an array of these dimensions')
               ...
               call mprintf(.true.,STDOUT,' allocate alt_topo for topo ' )
                    ...
                    allocate(alt_topo(dim1size-6,dim2size-6))
                    allocate(alt_stdh(dim1size-6,dim2size-6))
      do funit_three=10,100
         inquire(unit=funit_three, opened=is_used)
         if (.not. is_used) exit
      end do
               call mprintf(.true.,STDOUT,' HERE3 opening funit_three %i ' ,i1=funit_three)
                    open(unit=funit_three,file='mnts',form='unformatted')
                    read(funit_three) alt_topo,alt_land,alt_stdh
                    close(unit=funit_three)

                     do J=1,dim2size-6
                     do I=1,dim1size-6
                       field(I,J,1)=alt_topo(I,J)
                     enddo
                     enddo
               ...
               call mprintf(.true.,STDOUT,' call write_field for %s ', s1=trim(fieldname))
	...
	print*, 'working CATEGORICAL FIELD'
                  ...
                  if (.not. only_save_dominant) then
                     field_count = field_count + 1
                     call mprintf(.true.,LOGFILE,'(a) Processing field %i of %i (%s)', &
                                  i1=field_count,i2=NUM_FIELDS-NUM_AUTOMATIC_FIELDS,s1=fieldname)
                  else
                     field_count = field_count + 1
                     call mprintf(.true.,LOGFILE,'(b) Processing field %i of %i (%s)', &
                                  i1=field_count,i2=NUM_FIELDS-NUM_AUTOMATIC_FIELDS,s1=domname)
                  end if
                  ...
                  if (idomcatstatus == 0) then
                     call mprintf(.true.,STDOUT,' (c) Processing %s', s1=trim(domname))
