...
module source_data_module
   ...
   use gridinfo_module, only: use_igbp
   ...
   subroutine get_datalist()
    ...
    20 rewind(funit)
      ...
      allocate(source_fieldname(num_entries)) ; source_fieldname(:) = ' '
      ...
      allocate(source_dominant_category(num_entries)) ; source_dominant_category(:) = ' ' 
      ...
      allocate(source_units(num_entries)) ; source_units(:) = ' '
   ...
   recursive subroutine get_next_output_fieldname(field_name, ndims, &
      ...
      if (output_field_state == RETURN_LANDMASK) then
        ...
        if (trim(field_name) == 'LANDUSEF') then
        if (use_igbp) max_cat=20
        endif
                  end if
...
!	print*, 'recursive call (d)'
         ...
         if (output_field_state == RETURN_DOMCAT) then
            next_field = next_field - 1
!	print*, 'call get_next_fieldname(B)'
            call get_next_fieldname(field_name, istatus)
         else
            call get_landmask_field(field_name, is_water_mask, landmask, istatus)
         end if
...
!	print*, 'recursive call (e)'
         ...
         if (output_field_state == RETURN_DFDX) then
            next_field = next_field - 1
!	print*, 'call get_next_fieldname(C)'
            call get_next_fieldname(field_name, istatus)
         else
            call get_landmask_field(field_name, is_water_mask, landmask, istatus)
         end if
...
!	print*, 'recursive call (f)'
         ...
         if (output_field_state == RETURN_DFDY) then
            next_field = next_field - 1
!	print*, 'call get_next_fieldname(D)'
            call get_next_fieldname(field_name, istatus)
         else
            call get_landmask_field(field_name, is_water_mask, landmask, istatus)
         end if
