...
MODULE module_mp_wsm6
   ...
   REAL, PARAMETER, PRIVATE :: eacrc = 0.5        
...
CONTAINS
  ...
  SUBROUTINE wsm6(th, q, qc, qr, qi, qs, qg                        &
                 ...
                 ,snow, snowncv                                    &
                 ,sr                                               &
                 ,refl_10cm, diagflag, diagflagalt, do_radar_ref   &
                 ,graupel, graupelncv                              &
  ...
  INTEGER ::               i,j,k, lim_alt
      ...
      DO j=jts,jte
         ...
         if ( ( diagflag .or. diagflagalt ).and. do_radar_ref == 1) then
            DO I=its,ite
               TCOND=0.
               DO K=kts,kte
                  ...
                  qs1d(k)=qs(i,k,j)
                  qg1d(k)=qg(i,k,j)
                  TCOND=TCOND+qv1d(k)+qr1d(k)+qs1d(k)+qg1d(k)
               ENDDO



        IF (TCOND .ge. 1.e-12) THEN
          ...
          if (diagflag) then
               ...
               enddo
          elseif (diagflagalt) then




               lim_alt=16
               call refl10cm_wsm6 (qv1d(lim_alt:lim_alt), qr1d(lim_alt:lim_alt), &
                      qs1d(lim_alt:lim_alt), qg1d(lim_alt:lim_alt),              &
                       t1d(lim_alt:lim_alt), p1d(lim_alt:lim_alt),               &
                       dBZ(lim_alt:lim_alt), lim_alt, lim_alt, i, j)
               do k = kts, kte
                  refl_10cm(i,k,j) = -35.
               enddo
               refl_10cm(i,lim_alt,j)=max(-35., dbz(lim_alt))
          else
               do k = kts, kte
               refl_10cm(i,k,j) = -35.
               enddo
  ...
  IMPLICIT NONE
   ...
   g3pbg = rgmma(bvtg3)
   g4pbg = rgmma(bvtg4)
   pacrg = eacrc*pi*n0g*avtg*g3pbg*.25
   g5pbgo2 = rgmma(bvtg2)
   pvtg = avtg*g4pbg/6.
   precg1 = 2.*pi*n0g*.78
