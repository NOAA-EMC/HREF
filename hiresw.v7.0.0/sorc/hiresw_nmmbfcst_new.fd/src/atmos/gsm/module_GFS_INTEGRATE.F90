...
!         -----------------------------------------------
      ...
      if (nfhmax_hf > 0) then
        if (currtime <= starttime+output_hfmax) then
          LALARM = ESMF_AlarmIsRinging(alarm=ALARM_OUTPUT, rc = RC) .or.  &
                 ESMF_AlarmIsRinging(alarm=ALARM_OUTPUT_HF,rc = Rc)
        else
          LALARM = ESMF_AlarmIsRinging(alarm=ALARM_OUTPUT, rc = RC)
        endif
      else
        LALARM = ESMF_AlarmIsRinging(alarm=ALARM_OUTPUT, rc = RC)
      endif

      output2: IF( LALARM .and. LWRTGRDCMP) THEN    !<-- history output alarm
                                                    !    --------------------
                 CALL WRITE_ASYNC_GFS(WRT_COMPs,  exp_gfs_dyn              &
                                     ,imp_gfs_wrt,exp_gfs_wrt              &
                                     ,CLOCK_GFS                            &
                                     ,MYPE                                 &
                                     ,WRITE_GROUP_READY_TO_GO)
                 CALL ESMF_VMBarrier(VM_local, rc = RC)
      END IF output2
!
#endif
