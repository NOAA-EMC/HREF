...
! Work-around for Intel 13.x compilers with ESMF 310rp5, use specific
! routine instead of the overloaded interface:

      CALL ESMF_StateGetLogicalAttr(imp_gfs_dyn, 'Cpl_flag', Cpl_flag1, rc = rc1)
...
! Work-around for Intel 13.x compilers with ESMF 310rp5, use specific
! routine instead of the overloaded interface:
     CALL ESMF_StateSetReal8Attr(state    =IMP_STATE_WRITE                   &  !<-- The Write component import state
