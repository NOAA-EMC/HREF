...
!  -> Mask
!     ----
          findex = QueryFieldList(ImportFieldsList,
     &                           'land_mask')
          if (importData(1,1,findex) > -99999.0) then
...
!     ------
     ...
     &             'ice_fraction')
          if (importData(1,1,findex) > -99999.0) then
            do j = 1, lats_node_r
              do i = 1, lonr
                if (importData(i,j,findex) > 0.0001) then
                  if (aoi_fld%slimskin(i,j) < 3.1 .and.
     &                aoi_fld%slimskin(i,j) > 2.9) then
                   if (sfc_fld%slmsk(i,j) < 0.1 .or.
     &                 sfc_fld%slmsk(i,j) > 1.9) then
     ...
     &              aoi_fld%slimskin(i,j) < 4.1) then
                     aoi_fld%ULWSFCIN=-importData(i,j,findex)
                endif
              enddo
            enddo
          endif
