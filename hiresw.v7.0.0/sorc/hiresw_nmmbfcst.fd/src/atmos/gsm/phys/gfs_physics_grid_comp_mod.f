...
!------
      ...
      use ESMF
      use NUOPC
      use module_CPLFIELDS, only: inst_sea_surf_temp, &
...
!--get init time
      ...
      if(fhini/=0) int_state%restart_step=.true.
...
! Whenever the GSM run method is called, the "inst_sea_surf_temp" Field
! holds the latest SST from the OCN component.

! However, the "inst_sea_surf_temp" Field is only available if the SST import
! field was connected, and "inst_sea_surf_temp" was created.

  if (NUOPC_IsCreated(inst_sea_surf_temp)) then

    ! The "inst_sea_surf_temp" Field is defined on a non-reduced Gaussian grid
    ...
    buffer = -999000. - real(me)  ! identifiable init value
    call ESMF_FieldGather(inst_sea_surf_temp, buffer, rootPet=0, rc=rc)
    ...
    bufferSplit = -888000. - real(me) ! identifiable init value
    call split2d_phys_r8(buffer,bufferSplit,global_lats_ptr,int_state%nodes-1)

    ! Step 3: Reduce local blocks along the longitude dimension

    int_state%sstForGSM = -777000. - real(me) ! identifiable init value
    kmsk = 0  ! no masking
    call interpred_phys(1, kmsk, bufferSplit, int_state%sstForGSM, &
                        global_lats_ptr, lonsperlat_ptr)
                        
    ! -> DONE: now "sstForGSM: holds the incoming SST in GSM representation.
