...
#--------------------------------------------------------------------------------------------
    if etopgt10000 is not None:
      # Get the next plot ready
       t1=time.clock()
       print('Working on ETOPGT10000 '+dom)
      ...
      #Now plot 
       ...
       cm.set_over('white')
       norm = c.BoundaryNorm(clevs, cm.N)
       cs = m.contourf(lons,lats,etopgt10000,clevs,cmap=cm,norm=norm,latlon=True)
       cs.set_clim(1.0,100.)
       cbar = m.colorbar(cs,location='bottom',size="2%",pad="5%",ticks=clevs,format='%.2f')
       cbar.ax.tick_params(labelsize=8.5)
       cbar.set_label('probability')
       plt.title(domid+core+' Prob Echo Top height > ~35K feet \n'+repr(date)+' '+grbtime+'Z cycle Fhr '+fhr+' Valid '+valpdy+' '+valcyc+'00Z')
       plt.savefig('./href_etopgt10000_'+dom+'_f'+fhr+'_'+domid+core+'.png',bbox_inches='tight')
       subprocess.call(['convert','-density', '100x100', 'href_etopgt10000_'+dom+'_f'+fhr+'_'+domid+core+'.png', 'href_etopgt10000_'+dom+'_f'+fhr+'_'+domid+core+'.gif'])
       subprocess.call(['rm','href_etopgt10000_'+dom+'_f'+fhr+'_'+domid+core+'.png'])

       t2 = time.clock()
       t3=round(t2-t1, 3)

#--------------------------------------------------------------------------------------------
    if etopgt10668 is not None:
      # Get the next plot ready
       t1=time.clock()
