...
if gribproj.lower()=='lambert':
  ...
  Lon0=0.5*(lon1+lon2)
elif gribproj.lower()=='regular_ll':
  lon1 = grbs[1]['longitudeOfFirstGridPointInDegrees']
  lon2 = grbs[1]['longitudeOfLastGridPointInDegrees']
  if lon1 >= 0 and lon2 < 0 and grbs[1].iDirectionIncrement > 0:
    lon2 = 360+lon2
  if lon1 >= 0 and lon2 < lon1 and grbs[1].iDirectionIncrement > 0:
    lon1 = lon1-360
  lat1 = grbs[1]['latitudeOfFirstGridPointInDegrees']
  lat2 = grbs[1]['latitudeOfLastGridPointInDegrees']
...
#  if grbs[1].packingType.startswith('grid_complex'):
#    # this is not strictly correct for gaussian grids,
#    # but the error is very small.
#    print 'using workaround for complex packing'
...
#  else:
  ...
  if lon2 < 0:
      print 'dealing with negative lon2 value'
      lons = np.linspace(lon1,lon2,nx)
...
#      print("%.2f llcrnrlat now(2) is ") % llcrnrlat
    ...
    if gribproj.lower()=='regular_ll':
      print 'here a'
      if dom=='PR':
        print 'defining PR here'
        llcrnrlat=13.
        llcrnrlon=-77.
        urcrnrlat=23.
        urcrnrlon=-61.
        m = Basemap(llcrnrlon=llcrnrlon,llcrnrlat=llcrnrlat,urcrnrlon=urcrnrlon,urcrnrlat=urcrnrlat,\
                rsphere=rearth,resolution='h',projection='cyl')
      elif dom=='HI':
        print 'defining HI here'
        llcrnrlat=16.5
        llcrnrlon=-162.2
        urcrnrlat=24.
        urcrnrlon=-152.5
...
#mptest    m.fillcontinents(color='#ddaa66', lake_color='#7777ff', zorder = 0)
    ...
    if dom != 'CONUS' and dom != 'AK' and dom != 'PR' and dom != 'HI' :
...
## combo ptype
    ...
    if ptype is not None:
       ...
       subprocess.call(['rm','href_ptype_'+dom+'_f'+fhr+'_'+domid+core+'.png'])
       t2 = time.clock()
       t3=round(t2-t1, 3)
       print("%.3f seconds to plot ptype "+dom) % t3
