...
try:
  emsl=grbs.select(parameterName="192",parameterCategory=3)[0].values
  print 'min/max emsl', np.min(emsl),np.max(emsl)
except:
  emsl=None
...
try:
    print 'trying to read max hourly stuff'
    #read max hrly UH
...
#mptest    m.fillcontinents(color='#ddaa66', lake_color='#7777ff', zorder = 0)
    ...
    if dom != 'CONUS' and dom != 'AK':
      ...
      if dom in ['NW','NC','NE','SW','SC','SE','Great_Lakes']: skip=12
    elif dom=='CONUS': 
      skip=int(150/dx)
    elif dom=='AK':
      skip=int(150/dx)+2
...
#      clab = plt.clabel(cs,clevs[::3],inline=1, fontsize=10,fmt='%.0f')
      ...
      subprocess.call(['rm','hiresw_max10mw_'+dom+'_f'+fhr+'_'+domid+core+'.png'])
...
#  
#      plt.savefig('./hiresw_gust_'+dom+'_f'+fhr+'_'+domid+core+'.png',bbox_inches='tight')
#      subprocess.call(['convert','-density', '100x100', 'hiresw_gust_'+dom+'_f'+fhr+'_'+domid+core+'.png', 'hiresw_gust_'+dom+'_f'+fhr+'_'+domid+core+'.gif'])
#      subprocess.call(['rm','hiresw_gust_'+dom+'_f'+fhr+'_'+domid+core+'.png'])
#  
#      t2 = time.clock()
#      t3=round(t2-t1, 3)
...
#      plt.savefig('./hiresw_uphl_'+dom+'_f'+fhr+'_'+domid+core+'.png',bbox_inches='tight')
#      subprocess.call(['convert','-density', '100x100', 'hiresw_uphl_'+dom+'_f'+fhr+'_'+domid+core+'.png', 'hiresw_uphl_'+dom+'_f'+fhr+'_'+domid+core+'.gif'])
#      subprocess.call(['rm','hiresw_uphl_'+dom+'_f'+fhr+'_'+domid+core+'.png'])
