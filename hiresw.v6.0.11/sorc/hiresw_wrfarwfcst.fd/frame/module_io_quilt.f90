...
33 CONTINUE
      IF ( retval < 0 ) THEN
        CALL wrf_error_fatal3("<stdin>",174,&
...
okay_to_w = .false.
      DO WHILE (.TRUE.)
        ...
        reduced_dummy = 0
         call recv_flag(reduced, mpi_comm_io_groups(1))
        obufsize = reduced(1)
...
mess)
        ...
        IF ( obufsize .GT. 0 ) THEN
          ...
          CALL collect_on_comm_debug("module_io_quilt.F",707, mpi_comm_io_groups(1), &
                                onebyte, &
                                dummy, 0, &
                                obuf, obufsize )
        ELSE
        ...
        DO WHILE ( retval )
          IF ( mytask_local .EQ. ntasks_local_group-1 ) THEN
            ...
            ALLOCATE( bigbuf(1) )
          ENDIF
          CALL collect_on_comm_debug2("module_io_quilt.F",937,Trim(VarName), &
...
mess)
        ...
        IF ( obufsize .GT. 0 ) THEN
          ALLOCATE( obuf( (obufsize+1)/itypesize ) )
          CALL collect_on_comm_debug("module_io_quilt.F",1722, mpi_comm_io_groups(1), &
                                onebyte, &
                                dummy, 0, &
                                obuf, obufsize )
        ELSE
...
SUBROUTINE wrf_quilt_open_for_write_begin( FileName , Comm_compute, Comm_io, SysDepInfo, &
  ...
  call send_flag(reduced, tasks_in_group-1, comm_io_group)
  CALL collect_on_comm_debug("module_io_quilt.F",3009, comm_io_group, &
                        onebyte, &
                        hdrbuf, hdrbufsize , &
                        dummy, 0 )
  Status = 0
...
SUBROUTINE wrf_quilt_open_for_write_commit( DataHandle , Status )
  ...
  call send_flag(reduced, tasks_in_group-1, comm_io_group)
  CALL collect_on_comm_debug("module_io_quilt.F",3086, comm_io_group, &
                        onebyte, &
                        hdrbuf, hdrbufsize , &
                        dummy, 0 )
  Status = 0
...
SUBROUTINE wrf_quilt_iosync ( DataHandle, Status )
  ...
  IF ( associated ( int_local_output_buffer ) ) THEN
    ...
    IF ( wrf_dm_on_monitor() ) reduced(2) = DataHandle
      call send_flag(reduced, tasks_in_group-1, comm_io_group)
    CALL collect_on_comm_debug("module_io_quilt.F",3254, comm_io_group, &
                          onebyte, &
...
SUBROUTINE wrf_quilt_set_time ( DataHandle, Data, Status )
  ...
  IF ( DataHandle .GE. 1 .AND. DataHandle .LE. int_num_handles ) THEN
    IF ( int_handle_in_use( DataHandle ) ) THEN
      ...
      IF ( wrf_dm_on_monitor() ) reduced(2) = DataHandle
      call send_flag(reduced, tasks_in_group-1, comm_io_group)
      CALL collect_on_comm_debug("module_io_quilt.F",3534, comm_io_group, &
                            onebyte, &
                            hdrbuf, hdrbufsize , &
                            dummy, 0 )
    ENDIF
...
SUBROUTINE wrf_quilt_put_dom_ti_real ( DataHandle,Element, Data, Count, Status )
  ...
  IF ( DataHandle .GE. 1 .AND. DataHandle .LE. int_num_handles ) THEN
    IF ( int_handle_in_use( DataHandle ) ) THEN
      ...
      IF ( wrf_dm_on_monitor() ) reduced(2) = DataHandle
      call send_flag(reduced, tasks_in_group-1, comm_io_group)
      CALL collect_on_comm_debug("module_io_quilt.F",3654, comm_io_group, &
                            onebyte, &
                            hdrbuf, hdrbufsize , &
                            dummy, 0 )
    ENDIF
...
SUBROUTINE wrf_quilt_put_dom_ti_integer ( DataHandle,Element, Data, Count, Status )
  ...
  IF ( DataHandle .GE. 1 .AND. DataHandle .LE. int_num_handles ) THEN
    IF ( int_handle_in_use( DataHandle ) ) THEN
      ...
      IF ( wrf_dm_on_monitor() ) reduced(2) = DataHandle
      call send_flag(reduced, tasks_in_group-1, comm_io_group)
      CALL collect_on_comm_debug("module_io_quilt.F",3816, comm_io_group, &
                            onebyte, &
                            hdrbuf, hdrbufsize , &
                            dummy, 0 )
    ENDIF
...
SUBROUTINE wrf_quilt_put_dom_ti_char ( DataHandle, Element, Data, Status )
  ...
  IF ( DataHandle .GE. 1 .AND. DataHandle .LE. int_num_handles ) THEN
    IF ( int_handle_in_use( DataHandle ) ) THEN
      ...
      IF ( wrf_dm_on_monitor() ) reduced(2) = DataHandle
      call send_flag(reduced, tasks_in_group-1, comm_io_group)
      CALL collect_on_comm_debug("module_io_quilt.F",3989, comm_io_group, &
                            onebyte, &
                            hdrbuf, hdrbufsize , &
                            dummy, 0 )
    ENDIF
...
SUBROUTINE wrf_quilt_put_var_ti_char ( DataHandle,Element, Varname, Data, Status )
  ...
  IF ( DataHandle .GE. 1 .AND. DataHandle .LE. int_num_handles ) THEN
    IF ( int_handle_in_use( DataHandle ) ) THEN
      ...
      IF ( wrf_dm_on_monitor() ) reduced(2) = DataHandle
      call send_flag(reduced, tasks_in_group-1, comm_io_group)
      CALL collect_on_comm_debug("module_io_quilt.F",4523, comm_io_group, &
                            onebyte, &
                            hdrbuf, hdrbufsize , &
                            dummy, 0 )
    ENDIF
...
"frame/module_io_quilt.F: wrf_quilt_write_field: DataHandle not opened" )
  ...
  IF ( .NOT. okay_to_write( DataHandle ) ) THEN
      ...
      IF ( wrf_dm_on_monitor() ) reduced(2) = DataHandle
      call send_flag(reduced, tasks_in_group-1, comm_io_group)
      CALL collect_on_comm_debug("module_io_quilt.F",4947, comm_io_group, &
                            onebyte, &
                            hdrbuf, hdrbufsize , &
                            dummy, 0 )
  ELSE
