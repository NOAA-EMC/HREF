  ...
  integer, dimension( ims:ime, jms:jme ) , &
   ...
   do i = its,ite
      ...
      if (exch_temfx(i) > 1.0e-12) then
         qt(i,1) = qfx(i) / exch_temfx(i) + qvx(i,1)
      else
         qt(i,1) = qvx(i,1)
      end if
      ...
      do kt = 1,kte-1
         ...
         if (N2(i,kt) > 0.) then
            linv(i,kt) = 1./karman / zt(i,kt) + abs(fCor(i)) / (Cf*ustrtilde(i,kt)) + sqrt(N2(i,kt))/(CN*ustrtilde(i,kt)) + 1./lasymp
         else
            linv(i,kt) = 1./karman / zt(i,kt) + abs(fCor(i)) / (Cf*ustrtilde(i,kt)) + 1./lasymp
         end if
...
200 lcl(i) = zm(i,lclidx(i))
   ...
   300 hct(i) = zm(i,hctidx(i))
         ...
         MFCTE(i,1) = mf_temfx(i,1) * (TEUPD(i,1)-te_temfx(i,1) + (TEUPD(i,2)-te_temfx(i,2) - (TEUPD(i,1)-te_temfx(i,1))) * log(zt(i,1)/znt(i))/log(zm(i,2)/znt(i)))
         do kt = 2,kte-1
