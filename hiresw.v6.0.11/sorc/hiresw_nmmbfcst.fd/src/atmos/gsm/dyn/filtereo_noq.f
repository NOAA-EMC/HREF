...
!
! program log
! 2011 02 20 : henry juang, code for time filter without tracer 
!              for mass_dp and ndsl advection
cc
...
!              dpme(indev,2,k) =         dpe (indev,2,k)
     ...
     x                        + filtb *(dpme(indev,1,k)+dpne(indev,1,k))
              dpme(indev,2,k) = filta * dpe (indev,2,k)
     x                        + filtb *(dpme(indev,2,k)+dpne(indev,2,k))
...
!              dpmo(indod,2,k) =         dpo (indod,2,k)
     ...
     x                        + filtb *(dpmo(indod,1,k)+dpno(indod,1,k))
              dpmo(indod,2,k) = filta * dpo (indod,2,k)
     x                        + filtb *(dpmo(indod,2,k)+dpno(indod,2,k))
