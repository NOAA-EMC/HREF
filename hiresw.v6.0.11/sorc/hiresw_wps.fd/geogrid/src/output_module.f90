...
! For HDF5 only
  ...
  integer, parameter :: WRF_GRIB2_ERR_READ              = -409
      ...
      character (len=MAX_FILENAME_LEN) :: output_fname, output_fname_bin
...
! BUG: What if we have no plans to write 3-d fields? We should take this into account.
      ...
      if (grid_type == 'C') then
         if (io_form_output == BINARY) then
            output_fname = trim(opt_output_from_geogrid_path)//'geo_em.d  .int'
            output_fname_bin = trim(opt_output_from_geogrid_path)//'geo_em.d  .bin'          
         endif
         if (io_form_output == NETCDF) output_fname_bin = trim(opt_output_from_geogrid_path)//'geo_em.d  .bin'          
...
! seems like real_dom_arry is full dimension even on multiple PE.
        ...
        write(6,*) 'not training, writing ' , trim(fields(i)%fieldname), &
                                               ed(1),ed(2),ed(3)
        write(6,*) 'size(real_dom_array): ', size(real_dom_array,dim=1), &
               size(real_dom_array,dim=2), size(real_dom_array,dim=3)
