...
!mp  LOGICAL, DIMENSION(int_num_handles) :: okay_to_write, &
!mp                                            int_handle_in_use, okay_to_commit

  LOGICAL, DIMENSION(int_num_handles) :: okay_to_write, int_handle_in_use, &
                                         okay_to_commit, only_for_read
...
! dont use first 8 handles
	...
	write(0,*) 'searching for handle...' , i
...
33    CONTINUE
      IF ( retval < 0 )  THEN
	write(0,*) 'quit search for handle, I= ', I
...
!! how do we set so open_status will be WRF_FILE_OPENED_FOR_READ ??
...
SUBROUTINE ext_int_inquire_opened ( DataHandle, FileName , FileStatus, Status )
  ...
  IF ( DataHandle .GE. 1 .AND. DataHandle .LE. int_num_handles ) THEN
    ...
    ELSE IF ( int_handle_in_use( DataHandle ) .AND. &
                  only_for_read( DataHandle )) THEN
...
!write(0,*)'>>> ',TRIM(locDateStr),' ',TRIM(CurrentDateInFile(DataHandle) ),' ',TRIM(locVarName)
	...
	write(0,*) 'in ext_int_get_next_time, DateStr now: ', DateStr
...
SUBROUTINE ext_int_get_dom_ti_integer ( DataHandle,Element,   Data, Count, Outcount, Status )
  ...
  IF ( int_valid_handle( DataHandle ) ) THEN
    IF ( int_handle_in_use( DataHandle ) ) THEN
      ...
      DO WHILE ( keepgoing )
        READ( unit=DataHandle , iostat = istat ) hdrbuf
	write(0,*) 'istat from read of hdrbuf: ', istat
        IF ( istat .EQ. 0 ) THEN
          code = hdrbuf(2)
	write(0,*) 'code is: ', code
          IF ( code .EQ. int_dom_ti_integer ) THEN
	write(0,*) 'call int_get_ti_header'
            CALL int_get_ti_header( hdrbuf, hdrbufsize, itypesize, rtypesize, &
                                    locDataHandle, locElement, Data, loccount, code )
!	write(0,*) 'return int_get_ti_header with locElement, Data: ', locElement, Data
	write(0,*) 'return int_get_ti_header with locElement: ', locElement
            IF ( TRIM(locElement) .EQ. TRIM(Element) ) THEN
              IF ( loccount .GT. Count ) THEN
          ...
          ELSE IF ( .NOT. ( code .EQ. int_dom_ti_real .OR. code .EQ. int_dom_ti_logical .OR. &
            ...
            BACKSPACE ( unit=DataHandle )
            keepgoing = .false. ; Status = 1
	write(0,*) 'in ext_int_get_dom_ti_integer, status set to 1 (FIRST PLACE)'
          ENDIF
        ELSE
          keepgoing = .false. ; Status = 1
	write(0,*) 'in ext_int_get_dom_ti_integer, status set to 1 (SECOND PLACE)'
        ENDIF
      ENDDO
    ENDIF
  ENDIF
...
SUBROUTINE ext_int_get_dom_td_char ( DataHandle,Element, DateStr,  Data,  Status )
  ...
  CHARACTER*(*) :: Data
  INTEGER ,       INTENT(OUT) :: Status
	write(0,*) 'doing nothing in ext_int_get_dom_td_char'
