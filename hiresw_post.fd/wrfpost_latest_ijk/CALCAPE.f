...
C     THE MODEL SURFACE IS LIFTED.  THE ARRAYS P1D, T1D, AND Q1D
C     ARE NOT USED.  FOR ITYPE=2 THE ARRAYS P1D, T1D, AND Q1D
C     DEFINE THE PARCEL TO LIFT IN EACH COLUMN.  BOTH TYPES OF
...
C   03-08-24  G MANIKIN    - ADDED LEVEL OF PARCEL BEING LIFTED
C                            AS OUTPUT FROM THE ROUTINE AND ADDED
C                            THE DEPTH OVER WHICH ONE SEARCHES FOR
C                            THE MOST UNSTABLE PARCEL AS INPUT
C     
C USAGE:    CALL CALCAPE(ITYPE,DPBND,P1D,T1D,Q1D,L1D,CAPE,
C                                CINS,PPARC)
C   INPUT ARGUMENT LIST:
C     ITYPE    - INTEGER FLAG SPECIFYING HOW PARCEL TO LIFT IS
C                IDENTIFIED.  SEE COMMENTS ABOVE.
C     DPBND    - DEPTH OVER WHICH ONE SEARCHES FOR MOST UNSTABLE PARCEL
...
C     PPARC    - PRESSURE LEVEL OF PARCEL LIFTED WHEN ONE SEARCHES
C                  OVER A PARTICULAR DEPTH TO COMPUTE CAPE/CIN
C     
C   OUTPUT FILES:
...
C--------------TRIAL MAXIMUM BUOYANCY LEVEL VARIABLES-------------------
      DO 20 KB=1,LM
chc       IF (ITYPE.EQ.2.AND.KB.GT.1) GOTO 20 
...
!$omp&         sqs10k,tbtk,tpspk,tqk,tthbtk,tthesk,tthk)
       DO 10 J=JSTA,JEND
        DO 10 I=1,IM
         ...
         PSFCK  =PMID(I,J,LMHK)
         PKL = PMID(I,J,KB)
chc         IF (ITYPE.EQ.1.AND.(PKL.LT.PSFCK-DPBND.OR.PKL.GT.PSFCK))
chc     &             GOTO 10	 
          IF (ITYPE.EQ.2.OR.
...
C--------------CHECK FOR MAXIMUM THETA E--------------------------------
 ...
 10    CONTINUE
       END IF
 20   CONTINUE 
C----FIND THE PRESSURE OF THE PARCEL THAT WAS LIFTED
!$omp  parallel do
!$omp& private(pkl)
        DO J=JSTA,JEND
        DO I=1,IM
...
C**
     ...
     1, PMID(1,JSTA_2L,L),PLQ,QQ,PP,RDPQ
     2,THE0Q,STHEQ,RDTHEQ,THESP,IPTB,ITHTB)
      ENDIF
