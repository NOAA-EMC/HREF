...
CHC        IF(HTM2D(I,J).GT.0.5.AND.
CHC     1     HTM2D(I-1,J)*HTM2D(I+1,J)
CHC     2    *HTM2D(I,J-1)*HTM2D(I,J+1)
CHC     3    *HTM2D(I-1,J-1)*HTM2D(I+1,J-1)
CHC     4    *HTM2D(I-1,J+1)*HTM2D(I+1,J+1).LT.0.5)THEN
C     
...
CHC      SLPX(I,J)=0.125*(PSLP(I-1,J)+PSLP(I+1,J)
CHC     1                +PSLP(I,J-1)+PSLP(I,J+1)
CHC     2                +4.*PSLP(I,J))
  460 CONTINUE
C
!$omp parallel do
     ...
     1'Debug:bad pslp,i,j,pslp,fis,pint(lm+1)= ',i,j,pslp(i,j)
     2,fis(i,j),pint(i,j,lm+1)
