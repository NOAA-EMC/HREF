...
C     
        ...
        ENDIF
c       if(i.eq.ii.and.j.eq.jj)print*,'Debug:L,TTV B SMOO= ',l,TTV(I,J) 
      ENDDO
      ENDDO
C
      KMM=KMNTM(L)
...
c     1          +TTV(I,J-1)+TTV(I,J+1)-4.0*TTV(I,J))/4.0)
C
c     if(i.eq.ii.and.j.eq.jj)print*,'Debug: L,TTV A S'
c    1,l,TTV(I,J),N
c     1,l,TNEW(I,J),N
  280 CONTINUE
C
...
CHC relaxation
CHC      IF(FIPRES(I,J,LP).LT.0.)GO TO 330
c      IF(FIPRES(I,J,LP).LT.0.)THEN  
c       DO LP=LSM,1,-1
c        IF (FIPRES(I,J) .LE. 0)
