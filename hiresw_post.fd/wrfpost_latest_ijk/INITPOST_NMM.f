...
C   03-07-25  H CHUANG - MODIFIED TO PROCESS NMM WRF
...
c      call ext_ncd_get_next_time(DataHandle, DateStr, Status)
...
C Getting tstart
      tstart=0.
      call ext_ncd_get_dom_ti_real(DataHandle,'TSTART',tmp
     + ,1,ioutcount,istatus)
      tstart=tmp    
...
C Getiing restart
      ...
      call ext_ncd_get_dom_ti_integer(DataHandle,'RESTARTBIN',itmp
     + ,1,ioutcount,istatus)
...
!             th ( i, j, l ) = dum3d ( i, j, l ) + 300.
      ...
      ll=lm/2
      do l=1,lm
      if(jj.ge. jsta .and. jj.le.jend)print*,'sample L,T= ',L
     &,T(ii,jj,l)
      end do
...
!
      DO L=1,LM
         DO I=1,IM
            DO J=JSTA_2L,JEND_2U
!              PMID(I,J,L)=EXP((ALOG(PINT(I,J,L))+
!     &               ALOG(PINT(I,J,L+1)))*0.5)
              PMID(I,J,L)=(PINT(I,J,L)+
     &               PINT(I,J,L+1))*0.5
...
C      call getVariable(fileName,DateStr,DataHandle,'RAINCV',DUMMY,
...
!           IFDY = NINT ( fdy*1000.)
     ...
     +        status='unknown')
           IMM = 2*IM-1
	   IF(TRUELAT2 .LT. 1.0e-5)THEN
	    write(10,1000) IMM,JM,LATSTART,LONSTART,CENLON,
     .               IFDX,IFDY,TRUELAT1,TRUELAT1
           ELSE
            write(10,1000) IMM,JM,LATSTART,LONSTART,CENLON,
     .               IFDX,IFDY,TRUELAT2,TRUELAT1
           END IF
