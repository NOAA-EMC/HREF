	...
	call retrieve_index(index,VarName,varname_all,nrecs,iret)
     ...
     + , mpi_status_ignore, ierr)
        if (ierr /= 0) then
	     ...
	     if(PSHLTR(I,J) .NE. PSHLTR(I,J))print*,'bad PSHLTR ',
     +          i,j,PSHLTR(I,J)
           enddo
          enddo
        end if
     ...
     +	     ,i,j,TSHLTR(i,j),PSHLTR(I,J)
             if(TSHLTR(I,J) .NE. TSHLTR(I,J))print*,'bad TSHLTR ',
     +          i,j,TSHLTR(I,J)	     
           enddo
          enddo
        end if
...
c reading VEGETATION TYPE 
     ...
     + , mpi_status_ignore, ierr)
        if (ierr /= 0) then
	     ...
	     if(i.eq.ii.and.j.eq.(jsta+jend)/2)print*,'sample IVGTYP',
     +         IVGTYP(I,J)  	     
           enddo
          enddo
        end if
     ...
     + , mpi_status_ignore, ierr)
        if (ierr /= 0) then
	     ...
	     if(i.eq.ii.and.j.eq.(jsta+jend)/2)print*,'sample ISLTYP',
     +         ISLTYP(I,J)  	     
           enddo
          enddo
        end if
...
!      VarName='HBOT'
     ...
     + , mpi_status_ignore, ierr)
        if (ierr /= 0) then
          ...
          do j = jsta_2l, jend_2u
           do i = 1, im
             HTOPD ( i, j ) = float(LM)-dummy2(i,j)+1.0
           enddo
          enddo
        end if
      end if
     ...
     + , mpi_status_ignore, ierr)
        if (ierr /= 0) then
          ...
          do j = jsta_2l, jend_2u
           do i = 1, im
             HBOTD( i, j ) = float(LM)-dummy2(i,j)+1.0
           enddo
          enddo
        end if
      end if
     ...
     + , mpi_status_ignore, ierr)
        if (ierr /= 0) then
          ...
          do j = jsta_2l, jend_2u
           do i = 1, im
             HTOPS( i, j ) = float(LM)-dummy2(i,j)+1.0
           enddo
          enddo
        end if
      end if
     ...
     + , mpi_status_ignore, ierr)
        if (ierr /= 0) then
          ...
          do j = jsta_2l, jend_2u
           do i = 1, im
             HBOTS ( i, j ) = float(LM)-dummy2(i,j)+1.0
           enddo
          enddo
        end if
      end if
