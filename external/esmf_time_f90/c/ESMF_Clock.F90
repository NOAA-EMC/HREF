! $Id: ESMF_Clock.F90,v 1.1 2003/06/27 20:49:22 michalak Exp $
!
...
#if 0
      rc = ESMF_SUCCESS
      IF ( clock%NumAlarms < MAX_ALARMS ) THEN
        clock%NumAlarms = clock%NumAlarms + 1
        Alarm%PrevRingTime = clock%CurrTime
        Alarm%Ringing = .FALSE.
        clock%Alarmlist(clock%NumAlarms) = Alarm
      ELSE
        rc = ESMF_FAILURE
      ENDIF
#else
...
#if 0
      DO i = 1, clock%NumAlarms
        alarm = clock%AlarmList(i)
        IF ( alarm%PrevRingTime + alarm%RingInterval <= clock%CurrTime ) THEN
write(0,*)' setting alarm ', i, ' to true'
           alarm%Ringing = .TRUE.
        ...
        ENDIF
        clock%AlarmList(i) = alarm
        clock%AlarmList(i)%PrevRingTime = alarm%PrevRingTime
      ENDDO
#else
      DO i = 1, MAX_ALARMS
        alarm = clock%AlarmList(i)
...
CALL print_a_time(clock%CurrTime)
CALL print_a_time(clock%StopTime)
      if ( clock%CurrTime > clock%StopTime ) THEN
