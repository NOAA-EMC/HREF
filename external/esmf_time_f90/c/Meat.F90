...
SUBROUTINE simplify( ni, di, no, do ) 
    ...
    DO np = 1, nprimes
      ...
      DO WHILE ( keepgoing )
        keepgoing = .FALSE.
        IF ( d/pr .NE. 0 .AND. n/pr .NE. 0 .AND. MOD(d,pr) .EQ. 0 .AND. MOD(d,pr) .EQ. 0 ) THEN
          d = d / pr
          n = n / pr
          keepgoing = .TRUE.
        ENDIF
...
IMPLICIT NONE
	...
	ENDIF
      ...
      IF ( time2%basetime%S .GE. 3600*24 ) THEN
	diy = diy + 1
      ENDIF
...
SUBROUTINE print_a_time( time )
   ...
   use ESMF_Timemod
   type(ESMF_Time) time
   character*ESMF_MAXSTR :: s
   integer rc
   CALL ESMF_TimeGetString( time, s, rc )
   print *,TRIM(s),time%basetime%Sn,time%basetime%Sd
   return
...
SUBROUTINE atotime ( str, time )
   ...
   if ( str(20:20) .eq. '.' ) then
     read(str,34) yr,mm,dd,c,h,m,s,ms
   else
     read(str,33) yr,mm,dd,c,h,m,s
     ms = 0
   endif
   CALL ESMF_TimeSet( time, YR=yr, MM=mm, DD=dd, H=h, M=m, S=s, MS=ms, rc=rc )
33 format (I4.4"-"I2.2"-"I2.2,A1,I2.2":"I2.2":"I2.2)
34 format (I4.4"-"I2.2"-"I2.2,A1,I2.2":"I2.2":"I2.2"."I4.4)
   return
